...
> **Scope note.** This guide describes the full development workspace. File paths it cites
> under `data/`, `state/`, and `scripts/diagnostics/` (including the conversion verification
> and tuning scripts) refer to that workspace; the raw source data under `data/` and most of
> the intermediate tuning artifacts under `state/` are not included in this repository (see
> the README). The verification script itself, `scripts/diagnostics/verify_fbi_conversion_rules.py`,
> is included and can be re-run against locally supplied source data.
...
* `[kaplan]`: stated in Jacob Kaplan's UCR book (https://ucrbook.com)
...
| **Part I / "legacy seven"** | Murder, rape, robbery, aggravated assault, burglary, larceny-theft, motor vehicle theft (arson is Part I but excluded from CrimeRisk-style products for reporting inconsistency). | N/A |
...
themselves. `[kaplan: ucrbook.com, "when agencies report only NIBRS the FBI converts and
releases that data as its SRS version"; verified in §3]`
...
| `state/cache/{offenses_known_year,nibrs_offense,nibrs_batch_header,...}/` | Per-year extractions of the zips above, made by this repo's loaders | Derived from the rows above, not independent sources | N/A |
...
* **Re-verifying the numbers in this document** is a development-workspace activity: in a
  ...
  figure (writing `state/qa/fbi_conversion_rules_verification.md`). The raw source data and
  most intermediate artifacts are not included in this repository.
...
* **Kaplan**: Jacob Kaplan's UCR Book, https://ucrbook.com: conversion and 2021 NIBRS
  transition history.
* **AGS methodology**: Applied Geographic Solutions' published CrimeRisk methodology
  documentation (www.appliedgeographic.com), compared across two vintages during
  development. Material changes noted between vintages: projection geography moved to
  census block; "compiles … via NIBRS" wording added; "only the largest cities" became
  "many cities"; the incident-data city list opened up (naming New York, Los Angeles,
  Chicago); modeling jurisdictions went from roughly 10,000 to roughly 13,000; and the
  claimed model fit moved from "over 85%" to "over 75%" of variance. The data source line
  ("FBI UCR, 2018–2024") was unchanged between vintages. AGS's own text is not reproduced
  here; this guide, not the AGS text, governs how this repository describes the FBI layer.
