...
from crimerisk.build_freshness import artifact_is_current, copy_if_changed, write_dependency_stamp
...
def _load_enabled_city_order(
    ...
    *,
    ...
    enabled = [city for city in enabled if city]
    excluded = {str(city).strip() for city in exclude_city_keys if str(city).strip()}
    unknown = sorted(excluded - set(enabled))
    if unknown:
        raise ValueError(
            "Excluded feed city keys are not active production feeds: " + ", ".join(unknown)
        )
    return [city for city in enabled if city not in excluded]
...
def _cached_city_keys(frame: pd.DataFrame) -> set[str]:
    if frame.empty:
        return set()
    if "city_key" in frame.columns:
        return set(frame["city_key"].dropna().astype(str).str.strip()) - {""}
    return {
        CITY_KEY_BY_NORMALIZED_NAME[str(city_name).strip().lower()]
        for city_name in frame.get("city_name", pd.Series(dtype="object")).dropna().tolist()
        if str(city_name).strip().lower() in CITY_KEY_BY_NORMALIZED_NAME
    }


def filter_excluded_city_keys(
    frame: pd.DataFrame,
    *,
    exclude_city_keys: tuple[str, ...],
) -> pd.DataFrame:
    """Remove active feed cities from either live or promoted city-truth surfaces."""
    excluded = {str(city).strip() for city in exclude_city_keys if str(city).strip()}
    if not excluded or frame.empty:
        return frame
    if "city_key" in frame.columns:
        keys = frame["city_key"].astype("string").fillna("").str.strip()
    else:
        keys = (
            frame.get("city_name", pd.Series("", index=frame.index, dtype="string"))
            .astype("string")
            .fillna("")
            .str.strip()
            .str.lower()
            .map(CITY_KEY_BY_NORMALIZED_NAME)
            .fillna("")
        )
    return frame.loc[~keys.isin(excluded)].copy()
...
def build_city_incident_share_surface(
    *,
    ...
    enabled_keys = _load_enabled_city_order(
        paths,
        exclude_city_keys=tuple(config.exclude_city_keys),
    )
...
def write_v2_city_incident_shares(
    *,
    ...
    combined_reconciliation_path: Path | None = None,
    config: CityIncidentShareBuildConfig = CityIncidentShareBuildConfig(),
    blocked_by: tuple[str, ...] | None = None,
) -> dict[str, object]:
    """Build the city incident share surface, its per-city reconciliation and the combined table.

    All three destinations are caller-supplied, because a caller that asks for a truncated or
    city-excluded surface is not building the production one and must not overwrite it. The
    evaluation harness passes a run-scoped directory for exactly that reason.
    """
    with stage_write_lock(paths=paths, stage="city_incident_shares", blocked_by=blocked_by):
        ...
        combined_reconciliation_path = combined_reconciliation_path or (
            paths.state_dir / "modeling" / f"city_incident_reconciliation_{int(config.year_end)}.parquet"
        )
        impls = _city_impls(paths)
        enabled_keys = _load_enabled_city_order(
            paths,
        ...
        cache_parameters = {
            ...
            "exclude_city_keys": sorted(set(config.exclude_city_keys)),
        }
        dependencies_current = artifact_is_current(
            out_path,
            dependency_paths,
        ...
        write_dependency_stamp(out_path, dependency_paths, parameters=cache_parameters)
        return {
            "rows": int(len(frame)),
            "out_path": str(out_path),
