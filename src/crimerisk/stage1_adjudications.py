...
Four deterministic rules resolve the Stage-1 defect classes at scale (cross-lane twin identity,
...
SUCCESSIONS_FILENAME = "ori_successions.csv"
...
def load_explicit_succession_rulings(paths, *, target_year: int) -> pd.DataFrame:
    """Reviewed reporter-key migrations that the municipal-only auto rule cannot see.

    These rows are identity facts, not value overrides.  They are limited to an old and
    a live ORI for the same named provider in the same state/county, with a declared
    effective year.  The consumer independently verifies the reporting-window break and
    current successor coverage before it may suppress the old key.
    """
    path = config_dir(paths) / SUCCESSIONS_FILENAME
    frame, provenance = _read_with_provenance_header(path)
    missing = [column for column in SUCCESSION_COLUMNS if column not in frame.columns]
    if missing:
        raise Stage1AdjudicationError(f"{path} is missing columns {missing}")
    out = frame.reindex(
        columns=list(dict.fromkeys(list(SUCCESSION_COLUMNS) + list(frame.columns)))
    ).copy()
    for column in out.columns:
        out[column] = out[column].astype("string").fillna("")
    duplicated_cases = sorted(
        set(out.loc[out["case_id"].duplicated(keep=False), "case_id"])
    )
    if duplicated_cases:
        raise Stage1AdjudicationError(
            f"{path} carries duplicate case_id {duplicated_cases}"
        )
    for column in ("superseded_ori", "successor_ori"):
        out[column] = out[column].str.strip().str.upper()
        blank = out.loc[out[column].eq(""), "case_id"].tolist()
        if blank:
            raise Stage1AdjudicationError(f"{path} has blank {column} for {blank}")
    same = out["superseded_ori"].eq(out["successor_ori"])
    if bool(same.any()):
        raise Stage1AdjudicationError(
            f"{path} maps an ORI to itself for {out.loc[same, 'case_id'].tolist()}"
        )
    duplicated_old = sorted(
        set(out.loc[out["superseded_ori"].duplicated(keep=False), "superseded_ori"])
    )
    if duplicated_old:
        raise Stage1AdjudicationError(
            f"{path} maps superseded ORIs more than once: {duplicated_old}"
        )
    out["effective_year"] = pd.to_numeric(
        out["effective_year"], errors="raise"
    ).astype(int)
    out["county_fips"] = out["county_fips"].str.zfill(3)
    out = out[out["effective_year"].le(int(target_year))].copy()
    if out.empty:
        return out
    out["source_registry"] = provenance["source_registry"]
...
def _year_scoped_registry_applies(paths, filename: str, *, target_year: int) -> bool:
    """Whether a year-specific evidence registry belongs to this annual build.

    A registry for another year remains valid historical evidence, but it is not an
    input to the requested year.  Reading it here still validates the provenance
    header and declared row count before the pipeline deliberately leaves it inert.
    The direct ``load_*_rulings`` functions continue to fail closed on a year
    mismatch so callers cannot accidentally ask to apply the wrong evidence.
    """
    _frame, provenance = _read_with_provenance_header(config_dir(paths) / filename)
    try:
        registry_year = int(provenance["target_year"])
    except ValueError as exc:
        raise Stage1AdjudicationError(
            f"{config_dir(paths) / filename} has invalid target_year "
            f"{provenance['target_year']!r}"
        ) from exc
    return registry_year == int(target_year)
...
def build_zero_missing_directives(paths, *, target_year: int) -> pd.DataFrame:
    ...
    if not _year_scoped_registry_applies(
        paths, ZEROS_FILENAME, target_year=int(target_year)
    ):
        return pd.DataFrame(columns=DIRECTIVE_COLUMNS)
...
def build_token_reporter_directives(paths, *, target_year: int) -> pd.DataFrame:
    ...
    if not _year_scoped_registry_applies(
        paths, TOKENS_FILENAME, target_year=int(target_year)
    ):
        return pd.DataFrame(columns=DIRECTIVE_COLUMNS)
...
def write_provenance_manifest(paths, *, target_year: int) -> dict[str, object]:
    """The four registries' provenance headers plus row counts, for build summaries."""
    manifest: dict[str, object] = {}
    for name, filename in (
        ("twins", TWINS_FILENAME),
        ("successions", SUCCESSIONS_FILENAME),
...
def registry_dependency_paths(paths) -> list[Path]:
    """Registries consumed while building observations and reporting regimes."""
    directory = config_dir(paths)
    return [
        directory / TWINS_FILENAME,
        directory / ZEROS_FILENAME,
        directory / TOKENS_FILENAME,
    ]


def succession_dependency_paths(paths) -> list[Path]:
    """Reporter migrations consumed only when building target-year controls."""
    return [config_dir(paths) / SUCCESSIONS_FILENAME]
