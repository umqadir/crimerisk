...
PHILADELPHIA_JURISDICTION_ID = "42:police_service:PAPEP0000"
...
WASHINGTON_DC_LIVE_START_YEAR = 2018
...
def _load_washington_dc_raw_snapshot(*, raw_dir: Path, start_year: int, end_year: int, force_refresh: bool = False) -> pd.DataFrame:
    ...
    effective_end_year = min(int(end_year), WASHINGTON_DC_MAX_VERIFIED_YEAR)
    if effective_end_year < effective_start_year:
        return pd.DataFrame(columns=list(WASHINGTON_DC_MINIMAL_COLUMNS))

    cache_path = _prepared_city_cache_path(
        raw_dir,
        f"washington_dc_minimal_snapshot_{effective_start_year}_{effective_end_year}.parquet",
    )
    if cache_path.exists() and cache_path.stat().st_size > 0 and not force_refresh:
        raw = pd.read_parquet(cache_path)
        if "objectid" not in raw.columns or "year" not in raw.columns:
            raw = pd.DataFrame()
    else:
        raw = pd.DataFrame()
    ...
    if raw.empty and "objectid" not in raw.columns:
        frames: list[pd.DataFrame] = []
        for source_year in range(effective_start_year, effective_end_year + 1):
            csv_path = _download_file(
                WASHINGTON_DC_DOWNLOAD_URL_TEMPLATE.format(year=source_year),
                raw_dir / f"washington_dc_{source_year}.csv",
                force_download=force_refresh,
            )
            frame = _load_washington_dc_snapshot_csv(csv_path)
            frame["year"] = int(source_year)
            frames.append(frame)
    ...
    raw["year"] = pd.to_numeric(raw["year"], errors="coerce").astype("Int64")
    raw = raw[raw["objectid"].ne("") & raw["year"].notna()].copy()
    raw = raw.drop_duplicates(["year", "objectid"], keep="last")
    return raw
...
def _washington_dc_murder_history_only(frame: pd.DataFrame, *, target_year: int) -> pd.DataFrame:
    year = pd.to_numeric(frame["year"], errors="coerce")
    keep = frame["offense"].astype("string").eq("murder") | year.eq(int(target_year))
...
def build_washington_dc_incident_share_surface(
    *,
    ...
    # The historical expansion is the murder estimator's evidence window. Keep the
    # existing target-year share surface for every volume offense so a murder repair
    # cannot silently retrain their nationwide residual allocation models.
    matched = _washington_dc_murder_history_only(matched, target_year=int(end_year))
    if matched.empty:
        warnings.warn(
...
def _prepare_st_louis_incidents(
    *,
    ...
    raw = _load_st_louis_raw_snapshot(
        raw_dir=raw_dir,
        start_year=effective_start_year,
        # The edition year and a local incident feed's verified end year are separate.
        # Retain the latest verified spatial texture when a newer edition is built; the
        # emitted rows carry their actual source year, so the age remains inspectable.
        end_year=effective_end_year,
        force_refresh=force_refresh,
    )
