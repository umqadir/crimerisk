...
def _normalize_optional_panel_schema(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    string_cols = [
        ...
        "raw_data_source_state_publication",
...
def _load_reporting_regimes(
    ...
    *,
    ...
    config = ReportingRegimeBuildConfig(year_start=2018, year_end=int(target_year))
...
STATE_PUBLICATION_CONTRACT_STATES = frozenset({"FL", "MS", "NY"})

LANE_SELECTION_REASON_OVERRIDE = "manual_source_override"
LANE_SELECTION_REASON_ONLY_LANE = "only_lane_present"
LANE_SELECTION_REASON_STATE_PUBLICATION_CONTRACT = "state_publication_contract"
...
def _agency_year_lane_signals(
    ...
    state_abbr = panel.get("state_abbr", pd.Series(pd.NA, index=panel.index))
    contract_by_ori = (
        pd.DataFrame({"ori9": panel["ori9"], "state_abbr": state_abbr})
        .drop_duplicates(subset=["ori9"], keep="first")
        .set_index("ori9")["state_abbr"]
        .astype("string")
        .str.upper()
        .isin(STATE_PUBLICATION_CONTRACT_STATES)
    )
    signals["is_state_publication_contract"] = (
        signals["source"].eq(STATE_PUBLICATION_SOURCE)
        & signals["ori9"].map(contract_by_ori).fillna(False).astype(bool)
    )
    return signals
...
def _lane_selection_reason(ranked: pd.DataFrame) -> pd.Series:
    ...
    reason.loc[contested] = np.select(
        [
            ...
            (
                win["is_state_publication_contract"]
                & ~run["is_state_publication_contract"]
            ).to_numpy(),
        ...
        [
            ...
            LANE_SELECTION_REASON_STATE_PUBLICATION_CONTRACT,
...
def build_agency_preferred_observations(
    *,
    paths: RepoPaths,
    year: int,
    reporting_year_end: int | None = None,
    ...
    state_pub = agency_obs[agency_obs["source"].eq(STATE_PUBLICATION_SOURCE)].rename(
        columns={
            ...
            "raw_data_source": "raw_data_source_state_publication",
        }
    )[
        key_cols
        + [
    ...
    reporting_regimes = _load_reporting_regimes(
        paths,
        target_year=int(reporting_year_end or year),
    ...
    selected_state_publication = panel["preferred_source"].eq(
        STATE_PUBLICATION_SOURCE
    ) & panel["raw_data_source_state_publication"].astype("string").eq(
        "ct_despp_town"
    )
    panel.loc[selected_state_publication, "preferred_raw_data_source"] = panel.loc[
        selected_state_publication, "raw_data_source_state_publication"
    ]
