...
def _assert_crosswalk_weights_partition_every_agency(crosswalk: pd.DataFrame) -> None:
    ...
    zero_weight_exclusion_relationships = {
        "covered_by_other_agency",
        "covered_or_superseded_provider",
        "reviewed_nonprovider_identity",
        "superseded_reporting_alias",
    }
    covered = (
        crosswalk.groupby("ori9", dropna=False)["relationship_type"].agg(
            lambda values: bool(len(values))
            and all(str(value) in zero_weight_exclusion_relationships for value in values)
        )
        if "relationship_type" in crosswalk.columns
        else pd.Series(False, index=sums.index)
    )
    bad = sums[((sums - 1.0).abs() > 1e-9) & ~(sums.eq(0.0) & covered.reindex(sums.index).fillna(False))]
    if bad.empty:
        return
    raise ValueError(
...
def assert_computed_current_year_fills_consumed(
    *,
    agency_estimates: pd.DataFrame,
    crosswalk: pd.DataFrame,
    ...
    """Every computed own-history fill reaches its jurisdiction control.

    This is deliberately separate from the full accounting identity.  A dropped
    estimate artifact can leave a plausible-looking zero control while the estimator
    has already computed the missing amount (the Savannah regression).  The explicit
    refusal escape hatch is machine-readable; no refusal is currently emitted by the
    mechanical fill lane.
    """
    if agency_estimates.empty:
        return
    estimates = agency_estimates.copy()
    source = estimates["agency_estimate_source"].astype("string").fillna("")
    adjustment = pd.to_numeric(
        estimates["agency_adjustment_count"], errors="coerce"
    ).fillna(0.0)
    fills = estimates[
        ~source.isin(LANE_GROUNDED_ESTIMATE_SOURCES) & adjustment.gt(tolerance)
    ].copy()
    if fills.empty:
        return
    cross = crosswalk.copy()
    ori_col = "ori9" if "ori9" in cross.columns else "ori"
    cross = cross.rename(columns={ori_col: "ori9"})
    fills = fills.merge(
        cross[["ori9", "jurisdiction_id", "weight"]], on="ori9", how="inner"
    )
    fills["_expected_fill"] = (
        pd.to_numeric(fills["agency_adjustment_count"], errors="coerce").fillna(0.0)
        * pd.to_numeric(fills["weight"], errors="coerce").fillna(0.0)
    )
    expected = fills.groupby(
        ["jurisdiction_id", "offense"], dropna=False, as_index=False
    )["_expected_fill"].sum()
    actual_columns = ["jurisdiction_id", "offense", "current_year_fill_count"]
    if "current_year_fill_refusal_reason" in targets.columns:
        actual_columns.append("current_year_fill_refusal_reason")
    check = expected.merge(targets[actual_columns], on=["jurisdiction_id", "offense"], how="left")
    actual = pd.to_numeric(check["current_year_fill_count"], errors="coerce").fillna(0.0)
    refusal_reason = check.get(
        "current_year_fill_refusal_reason", pd.Series(pd.NA, index=check.index)
    ).astype("string").str.strip()
    refused = refusal_reason.notna() & refusal_reason.ne("")
    bad = check[(check["_expected_fill"] - actual).abs().gt(tolerance) & ~refused]
    if not bad.empty:
        raise ValueError(
            f"{len(bad)} jurisdiction-offense computed current-year fill(s) were not "
            "consumed and carry no refusal reason: "
...
def build_jurisdiction_year_estimates(
    *,
    ...
    ct_town_registry = load_ct_town_coverage_registry(paths, require_exists=False)
    agency_panel, agency_estimates, ct_conservation = residualize_ctcsp_overlap(
        agency_panel=agency_panel,
        agency_estimates=agency_estimates,
        registry=ct_town_registry,
        target_year=int(config.target_year),
    )
    if not ct_conservation.empty:
        ct_audit_path = (
            paths.state_dir
            / "controls"
            / f"ct_despp_overlap_conservation_{int(config.target_year)}.parquet"
        )
        ct_audit_path.parent.mkdir(parents=True, exist_ok=True)
        ct_conservation.to_parquet(ct_audit_path, index=False)

    # The census is independent of the registry: a newly reporting federal ORI cannot
    # evade classification by simply being absent from the CSV. Installation agencies
    # retain their rows. National/service-wide and investigative-HQ rollups are removed
    # before both historical rollup and target aggregation.
    federal_registry = load_federal_ori_scope_registry(paths)
    federal_census = build_federal_ori_census(paths=paths, agency_panel=agency_panel)
    assert_nonzero_federal_oris_are_classified(
        census=federal_census,
        agency_estimates=agency_estimates,
        registry=federal_registry,
    )
    write_federal_ori_mass_audit(
        build_federal_ori_mass_audit(
            census=federal_census,
    ...
    agency_panel = exclude_national_hq_rows(
        agency_panel, registry=federal_registry
    )
    agency_estimates = exclude_national_hq_rows(
        agency_estimates, registry=federal_registry
    )
