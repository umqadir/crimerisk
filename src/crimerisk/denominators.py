...
# The same extract also carries a NIGHT surface. The legacy denominator never reads it; the v2
# exposure-normalizer lane (exposure_ensemble.py) does, and E3 found it the single best allocator
# for four of seven offenses. Attached on request only, so the legacy path is unchanged.
LANDSCAN_NIGHT_POP_COLUMN = "landscan_night_pop"
...
def _population_for_denominators(bg: pd.DataFrame, *, year: int) -> pd.Series:
    ...
    out = pd.Series(np.nan, index=bg.index, dtype=float)
    found = False
    for column in candidates:
        if column not in bg.columns:
            continue
        found = True
        values = pd.to_numeric(bg[column], errors="coerce").astype(float)
        missing = out.isna()
        if bool(missing.any()):
...
def attach_landscan_night_population(bg: pd.DataFrame, *, paths: RepoPaths) -> pd.DataFrame:
    """Attach the LandScan USA nighttime surface from the same extract as the daytime one.

    Read by the v2 exposure-normalizer lane only. The legacy denominator construction does not
    call this, so nothing about the promoted chain changes by its existence. Coverage is not
    filled to zero silently: the caller (`exposure_ensemble.build_exposure_normalizers`) fails
    closed on a missing block group, because a zero night leg where LandScan simply has no row
    would be indistinguishable from a genuinely empty cell.
    """
    out = bg.copy()
    out["bg_id"] = out["bg_id"].astype("string").str.zfill(12)
    if LANDSCAN_NIGHT_POP_COLUMN in out.columns and pd.to_numeric(
        out[LANDSCAN_NIGHT_POP_COLUMN],
        errors="coerce",
    ).notna().any():
        out[LANDSCAN_NIGHT_POP_COLUMN] = (
            pd.to_numeric(out[LANDSCAN_NIGHT_POP_COLUMN], errors="coerce").clip(lower=0.0)
        )
        return out
    out = out.drop(columns=[LANDSCAN_NIGHT_POP_COLUMN], errors="ignore")

    path = _landscan_bg_path(paths)
    if not path.exists():
        raise FileNotFoundError(
            "Missing LandScan USA block-group denominator extract: "
            f"{path}. The v2 exposure-normalizer lane reads both the day and night surfaces from it."
        )
    landscan = pd.read_parquet(
        path,
        columns=["block_group_geoid", LANDSCAN_NIGHT_POP_COLUMN],
    ).copy()
    landscan["bg_id"] = landscan["block_group_geoid"].astype("string").str.zfill(12)
    landscan[LANDSCAN_NIGHT_POP_COLUMN] = (
        pd.to_numeric(landscan[LANDSCAN_NIGHT_POP_COLUMN], errors="coerce").clip(lower=0.0)
    )
    landscan = (
        landscan.groupby("bg_id", dropna=False)[LANDSCAN_NIGHT_POP_COLUMN]
        .sum(min_count=1)
        .reset_index()
    )
    return out.merge(landscan, on="bg_id", how="left")
