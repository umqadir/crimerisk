...
KAPLAN_PACKAGED_MAX_YEAR = 2024

# Years past the Kaplan restack are read from the FBI's own Return A master file, parsed
# by scripts/pull/parse_fbi_ucr_return_a_master.py into a dated directory under data/.
RETURN_A_MASTER_DATA_SOURCE = "fbi_ucr_return_a_master"
RETURN_A_MASTER_PARSED_GLOB = "FBI-UCR-Return-A-Parsed-{year}-*"
    ...
    *,
    ...
    dependencies = [
        ...
        *succession_dependency_paths(paths),
        ...
        paths.data_dir
        / f"FBI-CDE-Agency-Rosters-{int(resolved.year_end)}"
        / "parsed"
        / f"agency_rosters_{int(resolved.year_end)}.parquet",
        paths.data_dir
    ...
    dependencies.extend(
        return_a_master_dependency_paths(
            paths,
            year_start=int(resolved.year_start),
            year_end=int(resolved.year_end),
        )
    )
    seen: set[Path] = set()
    ordered: list[Path] = []
...
def _table11_unit_key(value: object) -> str | None:
    """Stable key for county-unit labels in the CIUS Table 11 hierarchy."""
    ...
    tokens = [
        ...
        if token not in {"COUNTY", "S"}
    ]
    if len(tokens) >= 2 and tokens[0] == "ST" and tokens[-1].endswith("S"):
        tokens[-1] = tokens[-1][:-1]
    return " ".join(tokens) or None


def _match_cius_table11_agencies(
    rows: pd.DataFrame,
    *,
    agency_master: pd.DataFrame,
    explicit_successions: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """Match final CIUS state/tribal/other rows to durable ORIs.

    Exact, state-unique agency aliases handle ordinary rows. Unit-office rows
    require an exact composite or unit alias; the reviewed Maryland State Police
    succession registry supplies the county-unit identity that agency names alone
    cannot recover. Ambiguous rows remain unmatched.
    """
    table11 = rows[
        rows["publication_collection"].eq("table11_state_tribal_other")
    ].copy()
    if table11.empty:
        return table11.assign(ori9=pd.Series(dtype="string"))

    table11 = table11.reset_index(drop=True)
    table11["_match_row_id"] = np.arange(len(table11))
    table11["_unit_std"] = table11["publication_unit_raw"].map(norm_text)
    table11["_composite_std"] = (
        table11["publication_name_std"].fillna("")
        + " "
        + table11["_unit_std"].fillna("")
    ).str.strip()

    aliases = _build_agency_alias_frame(agency_master)

    def unique_candidates(alias_col: str, match_col: str) -> pd.DataFrame:
        candidates = aliases.rename(columns={"publication_name_std": alias_col})
        counts = candidates.groupby(
            ["state_abbr", alias_col], dropna=False
        )["ori9"].transform("nunique")
        candidates = candidates[counts.eq(1)][
            ["state_abbr", alias_col, "ori9"]
        ].drop_duplicates()
        return candidates.rename(columns={alias_col: match_col})

    matched_parts: list[pd.DataFrame] = []
    no_unit = table11[table11["_unit_std"].isna()].copy()
    if not no_unit.empty:
        exact = unique_candidates("_alias", "publication_name_std")
        exact_match = no_unit.merge(
            exact,
            on=["state_abbr", "publication_name_std"],
            how="inner",
        )
        exact_match["_match_priority"] = 10
        matched_parts.append(exact_match)

    with_unit = table11[table11["_unit_std"].notna()].copy()
    unresolved = with_unit
    if not unresolved.empty:
        composite = unique_candidates("_alias", "_composite_std")
        composite_match = unresolved.merge(
            composite, on=["state_abbr", "_composite_std"], how="inner"
        )
        composite_match["_match_priority"] = 20
        matched_parts.append(composite_match)
        unresolved = unresolved[
            ~unresolved["_match_row_id"].isin(composite_match["_match_row_id"])
        ].copy()
    if not unresolved.empty:
        unit = unique_candidates("_alias", "_unit_std")
        unit_match = unresolved.merge(
            unit, on=["state_abbr", "_unit_std"], how="inner"
        )
        unit_match["_match_priority"] = 10
        matched_parts.append(unit_match)

    successions = (
        explicit_successions.copy()
        if explicit_successions is not None
        else pd.DataFrame()
    )
    if not successions.empty:
        mdsp = successions[
            successions["case_id"].astype("string").str.startswith(
                "mdsp-ori-migration-"
            )
        ][["case_id", "successor_ori"]].copy()
        mdsp["_unit_key"] = mdsp["case_id"].astype("string").str.removeprefix(
            "mdsp-ori-migration-"
        ).str.replace("-", " ", regex=False).map(_table11_unit_key)
        mdsp = mdsp[mdsp["_unit_key"].ne("STATEWIDE")].rename(
            columns={"successor_ori": "ori9"}
        )
        state_police = table11[
            table11["state_abbr"].eq("MD")
            & table11["publication_name_std"].eq("STATE POLICE")
        ].copy()
        state_police["_unit_key"] = state_police["publication_unit_raw"].map(
            _table11_unit_key
        )
        state_police = state_police.merge(mdsp, on="_unit_key", how="inner")
        state_police["_match_priority"] = 100
        matched_parts.append(state_police)

        statewide = successions[
            successions["case_id"].eq("mdsp-ori-migration-statewide")
        ][["successor_ori"]].rename(columns={"successor_ori": "ori9"})
        statewide_rows = table11[
            table11["state_abbr"].eq("MD")
            & table11["publication_name_std"].eq(
                "MARYLAND STATE POLICE STATEWIDE"
            )
        ].copy()
        if len(statewide) == 1 and not statewide_rows.empty:
            statewide_rows["ori9"] = statewide.iloc[0]["ori9"]
            statewide_rows["_match_priority"] = 100
            matched_parts.append(statewide_rows)

    if not matched_parts:
        return pd.DataFrame(columns=list(table11.columns) + ["ori9"])
    matched = pd.concat(matched_parts, ignore_index=True)
    highest_priority = matched.groupby("_match_row_id")["_match_priority"].transform(
        "max"
    )
    matched = matched[matched["_match_priority"].eq(highest_priority)].copy()
    match_counts = matched.groupby("_match_row_id")["ori9"].transform("nunique")
    matched = matched[match_counts.eq(1)].drop_duplicates(
        subset=["_match_row_id"], keep="first"
    )
    return matched.drop(
        columns=[
            "_match_row_id",
...
def _promote_cius_agency_counts(
    counts: pd.DataFrame,
    *,
    agency_master: pd.DataFrame,
    year: int,
) -> pd.DataFrame:
    """Attach final CIUS counts to the durable agency identity.

    CIUS is itself a completed annual publication. It must not depend on a
    same-year Kaplan Return A row merely to obtain identity columns; that would
    discard the newest CIUS year whenever Kaplan has not yet packaged it.
    """

    if counts.empty:
        return pd.DataFrame()
    count_cols = ["ori9", "offense", "official_count"]
    if "publication_population" in counts.columns:
        count_cols.append("publication_population")
    official = counts[count_cols].copy()
    official = official.rename(columns={"official_count": "count"})
    promoted = official.merge(agency_master, on="ori9", how="inner")
    promoted["year"] = int(year)
    publication_population = pd.to_numeric(
        promoted.get(
            "publication_population", pd.Series(np.nan, index=promoted.index)
        ),
        errors="coerce",
    )
    agency_population = pd.to_numeric(
        promoted.get("population", pd.Series(np.nan, index=promoted.index)),
        errors="coerce",
    )
    promoted["population"] = publication_population.combine_first(agency_population)
    ...
    return promoted.drop(columns=["publication_population"], errors="ignore")
...
def _build_cius_summary_promotions(
    *,
    ...
    for year in range(int(config.year_start), int(config.year_end) + 1):
        ...
        if int(year) > KAPLAN_PACKAGED_MAX_YEAR:
            table11 = _match_cius_table11_agencies(
                cius_rows,
                agency_master=agency_master,
                explicit_successions=load_explicit_succession_rulings(
                    paths, target_year=int(year)
                ),
            )
            if not table11.empty:
                unique_official = table11.groupby(["ori9", "offense"])[
                    "official_count"
                ].transform("size")
                table11 = table11[unique_official.eq(1)].copy()
                table11_promoted = _promote_cius_agency_counts(
                    table11,
                    agency_master=agency_master,
                    year=year,
                )
                if promotions:
                    occupied = pd.concat(promotions, ignore_index=True)[
                        ["ori9", "year", "offense"]
                    ].drop_duplicates()
                    table11_promoted = table11_promoted.merge(
                        occupied.assign(_already_promoted=True),
                        on=["ori9", "year", "offense"],
                        how="left",
                    )
                    table11_promoted = table11_promoted[
                        table11_promoted["_already_promoted"].ne(True)
                    ].drop(columns="_already_promoted")
                if not table11_promoted.empty:
                    promotions.append(table11_promoted)

    if not promotions:
        return pd.DataFrame(columns=list(srs_obs.columns) + ["cius_reference_flag"])

    promoted_all = pd.concat(promotions, ignore_index=True)
    duplicate_key = promoted_all.duplicated(
        subset=["ori9", "year", "offense"], keep=False
    )
    if duplicate_key.any():
        conflicts = (
            promoted_all.loc[duplicate_key]
            .groupby(["ori9", "year", "offense"])["count"]
            .nunique(dropna=False)
        )
        if conflicts.gt(1).any():
            raise ValueError(
                "CIUS annual collections assign conflicting official counts to "
                "the same agency-offense-year: "
                + str(conflicts[conflicts.gt(1)].head(20).to_dict())
            )
...
def return_a_master_parsed_dir(paths: RepoPaths, *, year: int) -> Path | None:
    """Newest dated parse of the FBI Return A master for `year`, if one exists."""
    candidates = sorted(
        path
        for path in paths.data_dir.glob(RETURN_A_MASTER_PARSED_GLOB.format(year=year))
        if (path / f"return_a_master_annual_{year}.parquet").exists()
    )
    return candidates[-1] if candidates else None


def return_a_master_dependency_paths(
    paths: RepoPaths, *, year_start: int, year_end: int
) -> list[Path]:
    out: list[Path] = []
    for year in range(int(year_start), int(year_end) + 1):
        directory = return_a_master_parsed_dir(paths, year=year)
        if directory is None:
            continue
        out.append(directory / f"return_a_master_annual_{year}.parquet")
        monthly = directory / f"return_a_master_monthly_{year}.parquet"
        if monthly.exists():
            out.append(monthly)
    return out


def _build_return_a_master_annual_observations(
    *,
    paths: RepoPaths,
    config: ObservationBuildConfig,
    agency_master: pd.DataFrame,
) -> pd.DataFrame:
    """Return A master rows for the years the Kaplan restack does not reach.

    Same lane as the Kaplan rows (`srs_return_a_annual`, `as_released_return_a`) because
    it is the same file; only `raw_data_source` separates the FBI's own distribution
    from Kaplan's restack of it. Carrying it means a target year no longer depends on
    the CIUS publication bundle alone, and -- unlike that bundle, which has no month
    field and is therefore stamped twelve months everywhere -- it brings the agency's
    real submission count for the year.
    """
    first_year = max(int(config.year_start), KAPLAN_PACKAGED_MAX_YEAR + 1)
    last_year = int(config.year_end)
    ...
    for year in range(first_year, last_year + 1):
        ...
        annual_frames.append(
            pd.read_parquet(directory / f"return_a_master_annual_{year}.parquet")
        )
        monthly_path = directory / f"return_a_master_monthly_{year}.parquet"
        if monthly_path.exists():
            monthly_frames.append(
                pd.read_parquet(
                    monthly_path,
                    columns=["ori7", "year", "month", "offense", "count", "reported"],
                )
            )
    ...
    annual = annual[pd.to_numeric(annual["months_reported"], errors="coerce").ge(1)]
    if annual.empty:
        return pd.DataFrame()
    ...
    for offense in OFFENSES_7:
        if offense not in wide.columns:
            wide[offense] = np.nan
    meta = (
        annual.groupby(["ori7", "year"], dropna=False)
        .agg(
            months_reported=("months_reported", "max"),
            months_missing=("months_missing", "min"),
            population=("population", "first"),
            agency_name_raw=("agency_name_raw", "first"),
        )
        .reset_index()
    )
    wide = wide.merge(meta, on=["ori7", "year"], how="left")
    wide["annual_part1_total"] = wide[list(OFFENSES_7)].sum(axis=1, min_count=1)

    if monthly_frames:
        monthly = pd.concat(monthly_frames, ignore_index=True)
        monthly = monthly[monthly["reported"].astype(bool)]
        per_month = (
            monthly.groupby(["ori7", "year", "month"], dropna=False)["count"]
            .sum()
            .reset_index(name="part1_total")
        )
        metrics = (
            per_month.groupby(["ori7", "year"], dropna=False)
            .agg(
                monthly_row_count=("month", "size"),
                monthly_part1_total=("part1_total", "sum"),
                nonzero_months=("part1_total", lambda s: int((s > 0).sum())),
                max_month_total=("part1_total", "max"),
            )
            .reset_index()
        )
        wide = wide.merge(metrics, on=["ori7", "year"], how="left")
    else:
        wide["monthly_row_count"] = np.nan
        wide["monthly_part1_total"] = np.nan
        wide["nonzero_months"] = np.nan
        wide["max_month_total"] = np.nan

    wide["max_month_share"] = np.where(
        pd.to_numeric(wide["monthly_part1_total"], errors="coerce").fillna(0) > 0,
        wide["max_month_total"] / wide["monthly_part1_total"],
        np.nan,
    )
    # The annual value is the sum of the same monthly cells, so this is structurally
    # zero here; it is carried because the batch detector reads it for every lane.
    wide["annual_month_diff_ratio"] = np.where(
        pd.to_numeric(wide["annual_part1_total"], errors="coerce").fillna(0) > 0,
        (
            pd.to_numeric(wide["monthly_part1_total"], errors="coerce")
            - pd.to_numeric(wide["annual_part1_total"], errors="coerce")
        ).abs()
        / pd.to_numeric(wide["annual_part1_total"], errors="coerce").clip(lower=1.0),
        np.nan,
    )
    wide["monthly_lumpiness_flag"] = (
        pd.to_numeric(wide["annual_part1_total"], errors="coerce").fillna(0)
        >= config.srs_lumpy_min_total
    ) & (
        (
            pd.to_numeric(wide["nonzero_months"], errors="coerce").fillna(12)
            <= config.srs_lumpy_nonzero_months_max
        )
        | (
            pd.to_numeric(wide["max_month_share"], errors="coerce").fillna(0)
            >= config.srs_lumpy_max_month_share
        )
        | (
            pd.to_numeric(wide["annual_month_diff_ratio"], errors="coerce").fillna(0)
            >= config.srs_monthly_annual_mismatch_ratio
        )
    )
    wide["quality_tier"] = _quality_tier_from_months(wide["months_reported"])
    wide.loc[
        wide["monthly_lumpiness_flag"] & wide["quality_tier"].eq("high"), "quality_tier"
    ] = "medium"
    wide.loc[
        wide["monthly_lumpiness_flag"] & wide["quality_tier"].eq("medium"),
        "quality_tier",
    ] = "low"
    wide["observation_weight"] = _weight_from_tier(wide["quality_tier"])

    # The master keys on the seven-character ORI. Take the nine-character identity from
    # the agency master where it is unambiguous and fall back to the FBI's own padding.
    ori_map = (
        agency_master[["ori7", "ori9"]]
        .dropna()
        .drop_duplicates()
        .groupby("ori7")["ori9"]
        .agg(lambda values: values.iloc[0] if values.nunique() == 1 else pd.NA)
        .reset_index()
        .rename(columns={"ori9": "ori9_master"})
    )
    wide = wide.merge(ori_map, on="ori7", how="left")
    wide["ori9"] = (
        wide["ori9_master"].astype("string").fillna(wide["ori7"].astype("string") + "00")
    )
    wide = wide.drop(columns=["ori9_master", "max_month_total"])

    long_df = _offense_long(
        wide[
            [
                "ori9",
                "ori7",
    ...
    long_df["raw_data_source"] = RETURN_A_MASTER_DATA_SOURCE
    long_df["cius_reference_flag"] = False
    long_df["state_exception_flag"] = False
    ...
    for col in ["ori7", "population", "agency_name_raw"]:
        master_col = f"{col}_agency_master"
        if master_col in long_df.columns:
            long_df[col] = long_df[col].fillna(long_df[master_col])
    return long_df
...
def _drop_state_publication_non_reports(
    ...
    *,
    ...
    machinery engages instead of a fabricated full-year zero. CT DESPP town
    rows are exempt: those are complete annual publication units that do not
    exist in the FBI panel, so absence from SRS is expected and a published
    all-zero seven-offense vector is a measured zero.
    """
    if promotions.empty:
        return promotions
    ...
    is_ct_despp_town = promotions.get(
        "raw_data_source", pd.Series(pd.NA, index=promotions.index)
    ).astype("string").eq(CT_DESPP_RAW_DATA_SOURCE)
    is_non_report = (
        agency_year_total.le(0.0)
        & months_for_rows.le(0.0)
        & ~is_ct_despp_town
    )
...
def build_agency_year_observations(
    *,
    ...
    ct_town_registry = load_ct_town_coverage_registry(paths)
    ct_agency_base = ct_town_agency_metadata_rows(ct_town_registry)
    if not ct_agency_base.empty:
        agency_base = pd.concat(
            [agency_base, ct_agency_base], ignore_index=True, sort=False
        ).drop_duplicates(subset=["ori9", "state_abbr"], keep="last")
    kaplan_config = replace(
        config,
        year_end=min(int(config.year_end), KAPLAN_PACKAGED_MAX_YEAR),
    )
    srs_obs = _build_srs_annual_observations(
        paths=paths,
        config=kaplan_config,
        agency_master=agency_master,
    )
    ...
    # Years past the Kaplan restack come from the FBI's own Return A master file.
    return_a_master_obs = _build_return_a_master_annual_observations(
        paths=paths,
        config=config,
        agency_master=agency_master,
    )
    if not return_a_master_obs.empty:
        return_a_master_obs = _normalize_agency_observation_schema(return_a_master_obs)
        summary_candidate_frames.append(return_a_master_obs)
        print(
            "build_v2_observations: Return A master lane "
            f"{len(return_a_master_obs):,} rows / "
            f"{return_a_master_obs['ori9'].nunique():,} agencies "
            f"for {sorted(set(return_a_master_obs['year'].dropna().astype(int)))}",
            flush=True,
        )
    # The Return A panel the publication-lane helpers consult is the whole Return A
    # panel, not just the years Kaplan packaged.
    srs_panel = _concat_agency_observation_frames(
        [srs_obs, return_a_master_obs]
        if not return_a_master_obs.empty
        else [srs_obs],
...
def write_v2_observations(
    *,
    ...
    with stage_write_lock(paths=paths, stage="observations", blocked_by=blocked_by):
        ...
        dependencies = observation_dependency_paths(paths, config=config)
        write_dependency_stamp(agency_out_path, dependencies)
