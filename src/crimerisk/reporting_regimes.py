...
def _load_srs_month_mask_detail(
    *,
    ...
    for year in _srs_month_detail_years(config):
...
def _srs_month_detail_years(config: ReportingRegimeBuildConfig) -> range:
    """Years present in the packaged Kaplan monthly archive."""
    return range(
        int(config.year_start),
        min(int(config.year_end), KAPLAN_PACKAGED_MAX_YEAR) + 1,
    )
...
def build_agency_year_reporting_regimes(
    *,
    ...
    srs_batched_annual = (
        ...
        | base["srs_annual_batch_detected"].astype("boolean").fillna(False).astype(bool)
    )
    ...
    *,
    ...
    with stage_write_lock(paths=paths, stage="reporting_regimes", blocked_by=blocked_by):
        ...
        write_dependency_stamp(
            out_path,
            reporting_regime_dependency_paths(paths, config=config),
        )
        return {
