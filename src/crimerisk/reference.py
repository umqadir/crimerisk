...
_ROSTER_MUNICIPAL_SUFFIX_RE = re.compile(
    r"\b(?:POLICE DEPARTMENT|DEPARTMENT OF POLICE|DEPARTMENT OF PUBLIC SAFETY|"
    r"PUBLIC SAFETY DEPARTMENT|PUBLIC SAFETY|POLICE|CITY MARSHAL(?:S OFFICE)?)\b$"
)
...
def _fbi_roster_tribal_oris(*, paths: RepoPaths, roster_year: int) -> set[str]:
    roster_path = (
        paths.data_dir
        / f"FBI-CDE-Agency-Rosters-{int(roster_year)}"
        / "parsed"
...
def _normalize_roster_agency_type(value: object) -> str:
    """Map the current FBI agency roster's type vocabulary onto the master schema."""
    text = _std_text(value)
    if not text:
        return "unknown"
    mapping = {
        ...
        "COUNTY": "sheriff",
        "STATE POLICE": "state_law_enforcement",
        "OTHER STATE AGENCY": "state_law_enforcement",
        "UNIVERSITY OR COLLEGE": "special_jurisdiction",
        "OTHER": "special_jurisdiction",
        "TRIBAL": "special_jurisdiction",
        "FEDERAL": "special_jurisdiction",
...
def normalize_roster_municipal_name(value: object) -> str | None:
    """Return the municipality label carried by a current-roster police agency name.

    This deliberately removes only terminal law-enforcement descriptors. Geographic
    type words such as ``TOWNSHIP`` and ``VILLAGE`` remain part of the identity.
    """
    text = _std_text(value)
    previous = None
    while text and text != previous:
        previous = text
        text = _ROSTER_MUNICIPAL_SUFFIX_RE.sub("", text).strip()
    return text or None


def _load_current_roster_identity(*, paths: RepoPaths, year: int) -> pd.DataFrame:
    path = (
        paths.data_dir
        / f"FBI-CDE-Agency-Rosters-{int(year)}"
        / "parsed"
    ...
    if not path.exists():
        return pd.DataFrame(columns=["ori9"])
    roster = pd.read_parquet(
        path,
        columns=["ori", "state_abbr", "agency_name", "agency_type_name", "counties"],
    ).rename(columns={"ori": "ori9"})
    roster["ori9"] = roster["ori9"].astype("string").str.strip().str.upper()
    roster = roster[roster["ori9"].notna() & roster["ori9"].ne("")].copy()
    roster = roster.drop_duplicates(subset=["ori9"], keep="first")
    roster["roster_agency_name_raw"] = roster["agency_name"].astype("string")
    roster["roster_agency_name_std"] = roster["agency_name"].map(_std_text)
    is_city = roster["agency_type_name"].astype("string").str.strip().eq("City")
    roster["roster_municipal_name_std"] = pd.Series(
        pd.NA, index=roster.index, dtype="string"
    )
    roster.loc[is_city, "roster_municipal_name_std"] = roster.loc[
        is_city, "agency_name"
    ].map(normalize_roster_municipal_name)
    roster["roster_agency_type_raw"] = roster["agency_type_name"].astype("string")
    roster["roster_agency_type_norm"] = roster["agency_type_name"].map(
        _normalize_roster_agency_type
    )
    roster["roster_counties_raw"] = roster["counties"].astype("string")
    roster["roster_state_abbr"] = roster["state_abbr"].map(_std_text)
    roster["roster_state_fips"] = roster["roster_state_abbr"].map(STATE_ABBR_TO_FIPS)
    roster["source_presence_roster"] = True
    return roster[
        [
            "ori9",
            "roster_state_abbr",
            "roster_state_fips",
            "roster_agency_name_raw",
            "roster_agency_name_std",
            "roster_municipal_name_std",
            "roster_agency_type_raw",
            "roster_agency_type_norm",
            "roster_counties_raw",
            "source_presence_roster",
        ]
    ]


def _target_year_published_roster_oris(
    *, paths: RepoPaths, year: int, roster: pd.DataFrame
) -> set[str]:
    """Roster ORIs independently witnessed by the target-year CIUS tables.

    The CDE roster endpoint is a current directory even when queried with a data
    year; it can contain departments formed after the annual publication. It may
    enrich an identity already in the historical universe, but a new ORI enters
    that universe only when the target-year CIUS publication itself names it.
    """
    if roster.empty:
        return set()
    try:
        from crimerisk.fbi_publications import parse_cius_local_rows

        rows = parse_cius_local_rows(paths, year=int(year))
    ...
    table8 = rows[rows["publication_collection"].eq("table8_city")][
        ["state_abbr", "publication_name_exact_std"]
    ].drop_duplicates()
    roster_city = roster[
        roster["roster_agency_type_raw"].eq("City")
        & roster["roster_municipal_name_std"].notna()
    ][["ori9", "roster_state_abbr", "roster_municipal_name_std"]].rename(
        columns={
            "roster_state_abbr": "state_abbr",
            "roster_municipal_name_std": "publication_name_exact_std",
        }
    )
    if not table8.empty and not roster_city.empty:
        candidates.append(
            table8.merge(
                _unique_roster_aliases(
                    roster_city, alias_column="publication_name_exact_std"
                ),
    ...
    table9 = rows[rows["publication_collection"].eq("table9_university")][
        ["state_abbr", "publication_name_std"]
    ].drop_duplicates()
    roster_university = roster[
        roster["roster_agency_type_raw"].eq("University or College")
        & roster["roster_agency_name_std"].notna()
    ][["ori9", "roster_state_abbr", "roster_agency_name_std"]].rename(
        columns={
            "roster_state_abbr": "state_abbr",
            "roster_agency_name_std": "publication_name_std",
        }
    )
    if not table9.empty and not roster_university.empty:
        candidates.append(
            table9.merge(
                _unique_roster_aliases(
                    roster_university, alias_column="publication_name_std"
                ),
    ...
    ][["state_abbr", "publication_name_std", "publication_unit_raw"]].drop_duplicates()
    if not table11.empty:
        table11["publication_unit_std"] = table11["publication_unit_raw"].map(
            _std_text
        )
        table11["publication_composite_std"] = (
            table11["publication_name_std"].fillna("")
            + " "
            + table11["publication_unit_std"].fillna("")
        ).str.strip()
        roster_other = roster[
            roster["roster_agency_name_std"].notna()
        ][["ori9", "roster_state_abbr", "roster_agency_name_std"]].rename(
            columns={"roster_state_abbr": "state_abbr"}
        )
        for publication_column in (
            "publication_name_std",
            "publication_unit_std",
            "publication_composite_std",
        ):
            aliases = roster_other.rename(
                columns={"roster_agency_name_std": publication_column}
            )
            candidates.append(
                table11.merge(
                    _unique_roster_aliases(
                        aliases, alias_column=publication_column
                    ),
                    on=["state_abbr", publication_column],
                    how="inner",
                )[["ori9"]]
            )
    if not candidates:
        return set()
    return set(pd.concat(candidates, ignore_index=True)["ori9"].astype(str))


def _unique_roster_aliases(
    frame: pd.DataFrame, *, alias_column: str
) -> pd.DataFrame:
    frame = frame[frame[alias_column].notna()].copy()
    if frame.empty:
        return frame
    counts = frame.groupby(["state_abbr", alias_column], dropna=False)[
        "ori9"
    ].transform("nunique")
    return frame[counts.eq(1)].drop_duplicates(
        ["state_abbr", alias_column, "ori9"]
    )


def _merge_current_roster_identity(
    df: pd.DataFrame,
    *,
    paths: RepoPaths,
    year: int,
    append_roster_oris: set[str] | None = None,
) -> pd.DataFrame:
    """Enrich the historical Kaplan identity universe with the target-year roster.

    Kaplan currently ends in 2024. The current FBI roster is therefore the only
    authoritative identity source for agencies first appearing in the 2025 annual
    publication. Existing historical fields are retained; roster fields are explicit.
    """
    roster = _load_current_roster_identity(paths=paths, year=year)
    if roster.empty:
        out = df.copy()
        out["source_presence_roster"] = False
        return out
    historical_oris = set(df["ori9"].dropna().astype(str))
    admitted_new = set(append_roster_oris or set())
    roster = roster[
        roster["ori9"].astype(str).isin(historical_oris | admitted_new)
    ].copy()
    out = df.merge(roster, on="ori9", how="outer", validate="one_to_one")
    out["state_abbr"] = out["state_abbr"].fillna(out["roster_state_abbr"])
    out["state_fips"] = out["state_fips"].fillna(out["roster_state_fips"])
    out["agency_name_raw"] = out["agency_name_raw"].fillna(
        out["roster_agency_name_raw"]
    )
    out["agency_name_std"] = out["agency_name_std"].fillna(
        out["roster_agency_name_std"]
    )
    out["agency_type_raw"] = out["agency_type_raw"].fillna(
        out["roster_agency_type_raw"]
    )
    unknown_type = out["agency_type_norm"].isna() | out["agency_type_norm"].eq(
        "unknown"
    )
    out.loc[unknown_type, "agency_type_norm"] = out.loc[
        unknown_type, "roster_agency_type_norm"
    ]
    out["source_presence_roster"] = (
        out["source_presence_roster"].astype("boolean").fillna(False).astype(bool)
    )
    for column in ("source_presence_srs", "source_presence_nibrs"):
        if column in out.columns:
            out[column] = out[column].astype("boolean").fillna(False).astype(bool)
    out["agency_type_norm"] = out["agency_type_norm"].fillna("unknown")
    return out
...
def build_agency_master(
    *,
    ...
    df["source_presence_srs"] = (
        df["source_presence_srs"].astype("boolean").fillna(False).astype(bool)
    )
    df["source_presence_nibrs"] = (
        df["source_presence_nibrs"].astype("boolean").fillna(False).astype(bool)
    )
    roster_identity = _load_current_roster_identity(paths=paths, year=int(year_end))
    published_roster_oris = _target_year_published_roster_oris(
        paths=paths,
        year=int(year_end),
        roster=roster_identity,
    )
    df = _merge_current_roster_identity(
        df,
        paths=paths,
        year=int(year_end),
        append_roster_oris=published_roster_oris,
    )
