...
from crimerisk.city_shares import filter_excluded_city_keys
...
def build_confidence_artifacts(
    *,
    ...
    controls_path = paths.state_dir / "controls" / f"jurisdiction_controls_{int(year)}.parquet"
    controls = pd.read_parquet(controls_path) if controls_path.exists() else pd.DataFrame()
    level_bg = _level_provenance_wide(
        component, controls, geo_col="bg_id", out_geo_col="block_group_geoid"
    )
    ...
    if not level_bg.empty:
        bg_metrics = bg_metrics.merge(level_bg, on=["state_fips", "block_group_geoid"], how="left")
    ...
    level_tract = _level_provenance_wide(
        component, controls, geo_col="tract_id", out_geo_col="tract_id"
    )
    ...
    if not level_tract.empty:
        tract_component_metrics = tract_component_metrics.merge(
            level_tract, on=["state_fips", "tract_id"], how="left"
        )
    for offense in OFFENSES_7:
...
def enrich_confidence_surfaces(
    *,
    ...
    bg = _drop_internal_confidence_columns(_append_confidence_tiers(bg, year=year))
    ...
    tract = _drop_internal_confidence_columns(_append_confidence_tiers(tract, year=year))
    return bg, tract
...
def _level_provenance_wide(
    ...
    *,
    ...
    """Propagate jurisdiction-level evidence provenance independently of share quality."""
    if component.empty or controls.empty:
        return pd.DataFrame(columns=["state_fips", out_geo_col])
    fields = [
        "level_admission_status", "level_admission_reason", "level_repair_mode",
        "level_repair_share", "external_check_status", "benchmark_conflict_kind",
        "benchmark_weight", "unresolved_level_flag",
    ]
    required = {"jurisdiction_id", "offense", *fields}
    if not required.issubset(controls.columns):
        return pd.DataFrame(columns=["state_fips", out_geo_col])
    work = component.copy()
    work["jurisdiction_id"] = work["jurisdiction_id"].astype("string")
    levels = controls[["jurisdiction_id", "offense", *fields]].copy()
    levels["jurisdiction_id"] = levels["jurisdiction_id"].astype("string")
    levels["offense"] = levels["offense"].astype("string")
    work = work.merge(levels, on=["jurisdiction_id", "offense"], how="left")
    work["_weight"] = _component_weights(work, geo_col=geo_col)
    work["state_fips"] = work["state_fips"].astype("string").str.zfill(2)
    work[geo_col] = work[geo_col].astype("string").str.zfill(12 if geo_col == "bg_id" else 11)
    keys = ["state_fips", geo_col, "offense"]
    dominant = (
        work.sort_values([*keys, "_weight", "jurisdiction_id"], ascending=[True, True, True, False, True], kind="mergesort")
        .drop_duplicates(keys, keep="first")
    )
    # This aggregation runs once for every geography/offense cell.  Keep it in
    # vectorized groupby operations: groupby.apply made the national build both
    # quadratic in Python overhead and emitted a pandas warning per cell.
    work["_weight"] = pd.to_numeric(work["_weight"], errors="coerce").fillna(0.0)
    work["_weighted_level_repair_share"] = (
        pd.to_numeric(work["level_repair_share"], errors="coerce").fillna(0.0)
        * work["_weight"]
    )
    work["_weighted_benchmark_weight"] = (
        pd.to_numeric(work["benchmark_weight"], errors="coerce").fillna(0.0)
        * work["_weight"]
    )
    work["_unresolved_level_flag"] = (
        work["unresolved_level_flag"].astype("boolean").fillna(False).astype("int8")
    )
    numeric = (
        work.groupby(keys, dropna=False)
        .agg(
            _weight=("_weight", "sum"),
            _weighted_level_repair_share=("_weighted_level_repair_share", "sum"),
            _weighted_benchmark_weight=("_weighted_benchmark_weight", "sum"),
            _unresolved_level_flag=("_unresolved_level_flag", "max"),
        )
        .reset_index()
    )
    positive_weight = numeric["_weight"].gt(0.0)
    numeric["level_repair_share"] = np.where(
        positive_weight,
        numeric["_weighted_level_repair_share"] / numeric["_weight"].replace(0.0, np.nan),
        0.0,
    )
    numeric["benchmark_weight"] = np.where(
        positive_weight,
        numeric["_weighted_benchmark_weight"] / numeric["_weight"].replace(0.0, np.nan),
        0.0,
    )
    numeric["unresolved_level_flag"] = numeric["_unresolved_level_flag"].astype(bool)
    numeric = numeric[
        [*keys, "level_repair_share", "benchmark_weight", "unresolved_level_flag"]
    ]
    dominant_fields = dominant[keys + [
        "level_admission_status", "level_admission_reason", "level_repair_mode",
        "external_check_status", "benchmark_conflict_kind",
    ]]
    long = dominant_fields.merge(numeric, on=keys, how="left")
    base = long[["state_fips", geo_col]].drop_duplicates().rename(columns={geo_col: out_geo_col})
    for offense in OFFENSES_7:
        one = long[long["offense"].eq(offense)].drop(columns="offense").rename(columns={geo_col: out_geo_col})
        one = one.rename(columns={field: f"{field}_{offense}" for field in fields})
        base = base.merge(one, on=["state_fips", out_geo_col], how="left")
    return base
...
def _load_residual_training_city_shares(
    *,
    ...
    df = filter_excluded_city_keys(
        df,
        exclude_city_keys=exclude_feed_city_keys,
    )
    ...
    if year_end is not None and "year" in df.columns:
        years = pd.to_numeric(df["year"], errors="coerce")
        df = df[years.notna() & years.le(int(year_end))].copy()
    return df
...
def _append_confidence_tiers(surface: pd.DataFrame, *, year: int = 2024) -> pd.DataFrame:
    out = surface.copy()
    for offense in OFFENSES_7:
        ...
        # Footprint-derived mass is modeled (estimated) mass — a source fact, and
        # it is disclosed at the same materiality threshold as benchmark
        # imputation. The old any-epsilon "coverage unverified" framing was doubt
        # disclosure, retired under the display contract.
        footprint_coverage_unverified = (
            _numeric_series(
                out, f"footprint_derived_count_share_{offense}", default=0.0
            )
            .fillna(0.0)
            .ge(BENCHMARK_IMPUTED_SHARE_LOW_CUTOFF)
        )
        spatial_low = (
            (~valid_denominator)
            | reliability.eq("low")
        ...
        spatial_tier = pd.Series("medium", index=out.index, dtype="string")
        spatial_tier.loc[spatial_low] = "low"
        spatial_tier.loc[spatial_high] = "high"
        out[f"spatial_share_reliability_tier_{offense}"] = spatial_tier

        admission = out.get(
            f"level_admission_status_{offense}",
            pd.Series("coverage_defective", index=out.index, dtype="string"),
        ).astype("string")
        repair_share = _numeric_series(out, f"level_repair_share_{offense}", default=0.0).fillna(0.0).clip(0.0, 1.0)
        unresolved = out.get(
            f"unresolved_level_flag_{offense}",
            pd.Series(False, index=out.index, dtype="boolean"),
        ).astype("boolean").fillna(False).astype(bool)
        level_low = (
            unresolved
            | admission.isin({"coverage_defective", "source_identity_failure", "unresolved_review"})
            | repair_share.ge(0.5)
            | benchmark_imputed
            | footprint_coverage_unverified
        )
        level_high = (
            ~level_low
            & admission.isin({"valid_complete_year", "corroborated_structural_zero"})
            & repair_share.le(1e-9)
            & imputed_share.le(1e-9)
        )
        level_tier = pd.Series("medium", index=out.index, dtype="string")
        level_tier.loc[level_low] = "low"
        level_tier.loc[level_high] = "high"
        out[f"level_reliability_tier_{offense}"] = level_tier
        rank = {"low": 0, "medium": 1, "high": 2}
        composite_rank = np.minimum(level_tier.map(rank).astype(int), spatial_tier.map(rank).astype(int))
        reverse = {value: key for key, value in rank.items()}
        out[f"confidence_tier_{offense}"] = composite_rank.map(reverse).astype("string")
        repair_mode = out.get(
            f"level_repair_mode_{offense}", pd.Series("none", index=out.index, dtype="string")
        ).astype("string")
        out[f"level_provenance_text_{offense}"] = [
            _level_provenance_text(a, r, u, i, f, year=year)
            for a, r, u, i, f in zip(
                admission.astype(str),
                repair_mode.astype(str),
                unresolved,
                imputed_share.to_numpy(dtype=float),
                footprint_coverage_unverified.to_numpy(dtype=bool),
                strict=True,
            )
        ]
        transient = (
...
def _level_provenance_text(
    admission: str,
    repair_mode: str,
    unresolved: bool,
    benchmark_imputed_share: float = 0.0,
    footprint_coverage_unverified: bool = False,
    *,
    year: int = 2024,
) -> str:
    label = str(int(year))
    if footprint_coverage_unverified:
        return f"{label} total includes modeled mass for footprint territory outside the submission"
    if float(benchmark_imputed_share) > 1e-9:
        return f"{label} total includes modeled benchmark mass for nonreporting territory"
    if admission == "valid_partial_lower_bound":
        return f"{label} total annualized from verified partial report"
    if repair_mode in {"decayed_own_history", "decayed_own_history_or_pooled"}:
        return f"{label} total repaired from recent agency history"
    if repair_mode in {"soft_benchmark_reconciliation", "pooled_silent_unit"}:
        return f"{label} total modeled for nonreporting territory"
    if repair_mode == "reason_coded_level_repair_plus_soft_benchmark":
        return f"{label} total combines a reason-coded repair and modeled nonreporting territory"
    if repair_mode in {"source_supported_replacement", "swap_reclassification"}:
        return f"{label} total repaired from a verified official source"
    if admission in {"coverage_defective", "source_identity_failure"}:
        return f"{label} filing failed level review; displayed level is not a complete observed total"
    if admission not in {"valid_complete_year", "corroborated_structural_zero"}:
        return f"{label} level is a statewide residual, not a complete local observation"
    return f"{label} total observed from complete official report"
