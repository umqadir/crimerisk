...
RARE_OFFENSE_INFORMATION_CONSTANT_GRID = (
    1e-1,
    3e-1,
    1e0,
    3e0,
...
RARE_OFFENSE_INFORMATION_VALIDATION_FOLDS = 5
RARE_OFFENSE_INFORMATION_VALIDATION_SEED = 20240728
# A numerical support floor, not a substantive crime-rate floor.  The model is
# trained on log1p(rate); clipping negative predictions to exactly zero created
# an artificial point mass of zero allocation weight across 12.2M residents.
MIN_POSITIVE_PRIOR_RATE_PER_100K = 1e-6


def _strictly_positive_rate_from_log1p_prediction(
    prediction: np.ndarray,
) -> np.ndarray:
    """Invert log1p predictions without introducing an exact-zero atom."""
    lower = float(np.log1p(MIN_POSITIVE_PRIOR_RATE_PER_100K))
    clipped = np.clip(np.asarray(prediction, dtype=float), lower, 12.0)
...
CENSUS_DHC_GQ_PATH = "Census-2020-DHC/parsed/dhc_group_quarters_block_groups.parquet"
HPMS_BG_VINTAGE_YEAR = 2024
NLCD_BG_VINTAGE_YEAR = 2023
BG_STRUCTURAL_FEATURE_CACHE_YEAR = 2024
ANNUAL_FEATURE_SUFFIX_PATTERN = re.compile(r"_(?:19|20)\d{2}$")


def annual_feature_key(value: object) -> str:
    """Stable policy key for a feature whose suffix is the release year."""
    return ANNUAL_FEATURE_SUFFIX_PATTERN.sub("_{year}", str(value))
...
def _base_exclude_feature_cols() -> set[str]:
    return {
        ...
        "census2020_gq_total",
        "census2020_gq_institutional",
        "census2020_gq_correctional",
        "census2020_gq_juvenile",
        "census2020_gq_nursing",
        "census2020_gq_college",
        "census2020_gq_military",
        "census2020_gq_other_institutional",
        "census2020_gq_other_noninstitutional",
    }
...
def bg_feature_dependency_paths(paths: RepoPaths, *, year: int) -> list[Path]:
    ...
    dependency_paths: list[Path] = [
        ...
        paths.data_dir / CENSUS_DHC_GQ_PATH,
        ...
        paths.state_dir
        / "modeling"
        / f"bg_structural_risk_features_{BG_STRUCTURAL_FEATURE_CACHE_YEAR}.parquet",
...
def _merge_frozen_structural_feature_cache(
    bg: pd.DataFrame,
    *,
    feature_cache_path: Path,
) -> pd.DataFrame:
    """Restore cleaned derived covariates without overriding rebuilt source fields.

    The pinned feature cache is a compact release input produced from the bulky road,
    HPMS, transit, and NLCD extracts.  Freshly built columns always win; the cache only
    supplies columns whose prepared source extract is no longer present locally.
    """
    if not feature_cache_path.exists():
        return bg

    cache = pd.read_parquet(feature_cache_path).copy()
    if "bg_id" not in cache.columns:
        raise ValueError(f"Frozen structural feature cache missing bg_id: {feature_cache_path}")
    cache["bg_id"] = cache["bg_id"].astype("string").str.zfill(12)
    if bool(cache["bg_id"].duplicated().any()):
        raise ValueError(f"Frozen structural feature cache has duplicate bg_id rows: {feature_cache_path}")

    missing_columns = [
        column
        for column in cache.columns
        if column != "bg_id" and column not in bg.columns
    ]
    if not missing_columns:
        return bg

    in_scope = bg["bg_id"].astype("string").str.zfill(12)
    cached_ids = set(cache["bg_id"].dropna().astype(str))
    missing_ids = sorted(set(in_scope.dropna().astype(str)) - cached_ids)
    if missing_ids:
        raise ValueError(
            "Frozen structural feature cache does not cover the release BG frame; "
            f"missing {len(missing_ids):,} ids, examples={missing_ids[:10]}"
        )

    return bg.merge(
        cache[["bg_id", *missing_columns]],
        on="bg_id",
        how="left",
        validate="one_to_one",
    )
...
def build_bg_feature_frame(*, paths: RepoPaths, year: int) -> pd.DataFrame:
    ...
    gq_path = paths.data_dir / CENSUS_DHC_GQ_PATH
    if not gq_path.exists():
        raise FileNotFoundError(
            f"Missing Census DHC group-quarters input: {gq_path}; "
            "run scripts/pull/pull_census_dhc_group_quarters.py"
        )
    gq = pd.read_parquet(gq_path).copy()
    gq["block_group_geoid"] = gq["block_group_geoid"].astype("string").str.zfill(12)
    if gq["block_group_geoid"].duplicated().any():
        raise ValueError(f"Duplicate GEOIDs in Census DHC group-quarters input: {gq_path}")
    bg = bg.merge(
        gq.rename(columns={"block_group_geoid": "bg_id"}),
        on="bg_id",
        how="left",
        validate="one_to_one",
    )
    gq_columns = [column for column in gq.columns if column != "block_group_geoid"]
    for column in gq_columns:
        bg[column] = pd.to_numeric(bg[column], errors="coerce").fillna(0.0).clip(lower=0.0)
    ...
    bg = _merge_frozen_structural_feature_cache(
        bg,
        feature_cache_path=(
            paths.state_dir
            / "modeling"
...
def _build_feature_selection(
    ...
    *,
    ...
    base_exclude.update(
        ...
        if re.fullmatch(r"population_(?:19|20)\d{2}", str(column))
    )
...
def _apply_feature_policy_exclusions(feature_cols: list[str], excluded_columns: set[str]) -> list[str]:
    ...
    excluded_keys = {annual_feature_key(col) for col in excluded_columns}
    return [col for col in feature_cols if annual_feature_key(col) not in excluded_keys]
...
def _feature_cols_for_offense(
    ...
    *,
    ...
    if not denominator_anchor or str(offense) != "burglary" or not burglary_exposure_duplicate_columns:
        return list(feature_cols)
    duplicate_keys = {annual_feature_key(col) for col in burglary_exposure_duplicate_columns}
    return [col for col in feature_cols if annual_feature_key(col) not in duplicate_keys]
...
def load_county_to_cbsa_delineation(paths: RepoPaths) -> pd.DataFrame:
    """County FIPS -> 2023 OMB CBSA delineation. Public: the rollup lane reads the same table."""
...
def _poisson_deviance(observed: np.ndarray, expected: np.ndarray) -> float:
    mu = np.clip(np.asarray(expected, dtype=float), 1e-9, None)
...
def select_rare_offense_information_constants(
    validation_tracts: pd.DataFrame,
    *,
    offenses: tuple[str, ...] = SPARSE_OFFENSES_DEFAULT,
    information_constant_grid: tuple[float, ...] = RARE_OFFENSE_INFORMATION_CONSTANT_GRID,
    validation_folds: int = RARE_OFFENSE_INFORMATION_VALIDATION_FOLDS,
    validation_seed: int = RARE_OFFENSE_INFORMATION_VALIDATION_SEED,
) -> tuple[dict[str, float], pd.DataFrame]:
    """Select rare-offense information constants by masked Poisson validation.

    ``validation_tracts`` contains cold-start tract predictions from the city evidence
    set. Required columns are ``validation_group_id``, ``offense``, ``observed_count``,
    ``incident_total``, ``model_share``, ``exposure_share`` and
    ``effective_model_information``. For a candidate K the group-level model weight is
    ``I / (I + K)``. The selector uses the same grid, five-fold masking, seed and
    out-of-fold Poisson-deviance objective as the benchmark-imputation pooling-constant
    selector. One K is selected per offense.

    Masking is by validation group, never by tract, so all tracts from a city/offense
    case stay in the same fold. The input predictions must already be cold-start; this
    function selects only the shrinkage constant and does not refit the upstream model.
    """
    required = {
        "validation_group_id",
        "offense",
        ...
        "model_share",
        "exposure_share",
        "effective_model_information",
    }
    missing = sorted(required - set(validation_tracts.columns))
    if missing:
        raise ValueError(f"rare-offense information validation missing columns: {missing}")
    if not information_constant_grid:
        raise ValueError("rare-offense information constant grid must not be empty")
    if int(validation_folds) < 2:
        raise ValueError("rare-offense information validation requires at least two folds")

    work = validation_tracts.copy()
    work["validation_group_id"] = work["validation_group_id"].astype("string")
    ...
    allowed = {str(offense) for offense in offenses}
    work = work[work["offense"].isin(allowed) & work["incident_total"].gt(0.0)].copy()
    if work.empty:
        raise ValueError("rare-offense information validation has no eligible rows")

    groups = (
        work[["validation_group_id", "offense"]]
        .drop_duplicates()
        .sort_values(["offense", "validation_group_id"], kind="mergesort")
        .reset_index(drop=True)
    )
    rng = np.random.default_rng(int(validation_seed))
    groups["fold"] = rng.integers(0, int(validation_folds), size=len(groups))
    work = work.merge(groups, on=["validation_group_id", "offense"], how="left", validate="many_to_one")

    rows: list[dict[str, float | int | str]] = []
    for raw_constant in information_constant_grid:
        constant = float(raw_constant)
        if not np.isfinite(constant) or constant <= 0.0:
            raise ValueError(f"rare-offense information constant must be finite and positive: {raw_constant!r}")
        information = work["effective_model_information"].to_numpy(dtype=float)
        model_weight = information / (information + constant)
        predicted_share = (
            model_weight * work["model_share"].to_numpy(dtype=float)
            + (1.0 - model_weight) * work["exposure_share"].to_numpy(dtype=float)
        )
        expected = predicted_share * work["incident_total"].to_numpy(dtype=float)
        for offense in offenses:
            offense_key = str(offense)
            offense_mask = work["offense"].eq(offense_key)
            for fold in range(int(validation_folds)):
                mask = offense_mask & work["fold"].eq(fold)
                if not bool(mask.any()):
                    continue
                rows.append(
                    {
                        "information_constant": constant,
                        "offense": offense_key,
                        "fold": int(fold),
                        "validation_group_count": int(work.loc[mask, "validation_group_id"].nunique()),
                        "observed_count": float(work.loc[mask, "observed_count"].sum()),
                        "poisson_deviance": _poisson_deviance(
                            work.loc[mask, "observed_count"].to_numpy(dtype=float),
                            expected[mask.to_numpy(dtype=bool)],
                        ),
                    }
                )
    validation = pd.DataFrame(rows)
    if validation.empty:
        raise ValueError("rare-offense information validation produced no fold scores")
    aggregate = (
        validation.groupby(["information_constant", "offense"], dropna=False)
        .agg(
            poisson_deviance=("poisson_deviance", "sum"),
            validation_group_count=("validation_group_count", "sum"),
            observed_count=("observed_count", "sum"),
            scored_fold_count=("fold", "nunique"),
        )
        .reset_index()
    )
    ...
    for offense in offenses:
        offense_key = str(offense)
        candidates = aggregate[aggregate["offense"].eq(offense_key)]
        if candidates.empty:
            raise ValueError(f"rare-offense information validation has no scores for {offense_key}")
        best = candidates.sort_values(
            ["poisson_deviance", "information_constant"],
            kind="mergesort",
        ).iloc[0]
        chosen[offense_key] = float(best["information_constant"])
    return chosen, validation.sort_values(
        ["offense", "information_constant", "fold"], kind="mergesort"
    ).reset_index(drop=True)
...
def _prepare_model_surface_context(
    *,
    ...
    county_to_cbsa = load_county_to_cbsa_delineation(paths)
...
def build_model_surface(
    *,
    ...
    for offense in OFFENSES_7:
        ...
        pred_rate = _strictly_positive_rate_from_log1p_prediction(pred_log_rate)
