...
from crimerisk.contract_coverage import (
    apply_contract_coverage,
    apply_footprint_reassignment_overrides,
    covered_by_source_path,
    load_covered_by_relationships,
)
from crimerisk.pa_police_service import (
    apply_pa_police_service_coverage,
    pa_police_service_registry_path,
)
...
def reference_layer_dependency_paths(
    *,
    ...
    dependencies = [
        ...
        paths.repo_root / "src" / "crimerisk" / "contract_coverage.py",
        paths.repo_root / "src" / "crimerisk" / "pa_police_service.py",
        paths.data_dir
        ...
        paths.repo_root / "configs" / "footprint_reassignment_overrides.csv",
    ]
    seen: set[Path] = set()
    ordered: list[Path] = []
...
def promote_v2_reference_inputs(
    *,
    ...
    for output_name, canonical_path, source_path in _iter_reference_input_promotions(paths=paths, config=resolved):
        ...
        copy_if_changed(source_path, canonical_path)
...
def build_v2_reference_layers(
    *,
    ...
    artifacts = build_reference_artifacts(
        ...
        target_year=int(resolved.year_end),
    )
    artifacts = apply_contract_coverage(
        artifacts,
        load_covered_by_relationships(paths, year=int(resolved.year_end)),
    )
    artifacts = apply_footprint_reassignment_overrides(artifacts, paths)
    artifacts = apply_pa_police_service_coverage(artifacts, paths=paths)
    return artifacts
    ...
    *,
    ...
    with stage_write_lock(paths=paths, stage="reference_layers", blocked_by=blocked_by):
        ...
        for _, path in outputs:
            write_dependency_stamp(path, dependencies)
        ...
        agency_master_oris = set(agency_master["ori9"].dropna().astype(str))
        crosswalk_oris = set(crosswalk["ori"].dropna().astype(str))
        return {
            ...
            "missing_crosswalk_ori": int(len(agency_master_oris - crosswalk_oris)),
            "state_publication_crosswalk_units": int(
                len(crosswalk_oris - agency_master_oris)
            ),
        }
