...
def load_ct_despp_town_annual_ags_rows(
    *,
    paths: RepoPaths,
    year: int = 2024,
) -> pd.DataFrame:
    """CT DESPP SRS-equivalent town lane keyed by state-publication unit.

    The canonical parsed extract is already one row per DESPP town unit and
    Part I offense.  Unlike FDLE, no name-to-FBI-ORI match is attempted: CTSP
    identifiers are state-publication units whose exclusive county-subdivision
    footprints come from ``configs/ct_town_coverage.csv``.
    """
    if int(year) not in CT_DESPP_SUPPORTED_YEARS:
        return _empty_state_publication_frame()
    source_path = get_ct_town_parsed_path(paths)
    if not source_path.exists():
        raise FileNotFoundError(
            "CT DESPP 2024 is a required state-publication lane; missing parsed "
            f"extract: {source_path}"
        )
    registry = load_ct_town_coverage_registry(paths)
    raw = validate_ct_town_parsed_rows(
        pd.read_csv(source_path), registry=registry, year=year
    )
    lane_units = set(
    ...
    raw = raw[raw["reporting_unit"].astype("string").isin(lane_units)].copy()
    grouped = (
        raw.rename(columns={"reporting_unit": "ori9"})
        .groupby(["ori9", "year", "offense"], dropna=False, as_index=False)["count"]
        .sum()
    )
    grouped["quarters_reported"] = 4.0
    grouped["months_reported"] = 12.0
    grouped["state_abbr"] = "CT"
    grouped["source"] = STATE_PUBLICATION_SOURCE
    grouped["raw_data_source"] = CT_DESPP_RAW_DATA_SOURCE
    return grouped.reindex(columns=STATE_PUBLICATION_COLUMNS)
...
def _build_state_publication_annual_ags_rows(
    *,
    ...
    frames = [
        load_ct_despp_town_annual_ags_rows(paths=paths, year=year),
...
def write_v2_state_publication_inputs(
    *,
    ...
    lane_supported_years = {
        "CT DESPP towns (CT)": tuple(CT_DESPP_SUPPORTED_YEARS),
