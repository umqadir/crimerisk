...
MUNICIPAL_TYPED_SUFFIXES: tuple[tuple[str, str], ...] = (
    ("CITY AND BOROUGH", "CITY_AND_BOROUGH"),
    ("CITY AND COUNTY", "CITY_AND_COUNTY"),
    ("CHARTER TOWNSHIP", "TOWNSHIP"),
    ("METROPOLITAN GOVERNMENT", "METROPOLITAN_GOVERNMENT"),
    ("UNIFIED GOVERNMENT", "UNIFIED_GOVERNMENT"),
    ("URBAN COUNTY", "URBAN_COUNTY"),
...
def norm_typed_municipal_name(value: object) -> str | None:
    """Normalize a municipal label while preserving its legal geography type."""
    text = norm_text(value)
    if not text:
        return None
    for suffix, kind in MUNICIPAL_TYPED_SUFFIXES:
        prefix = f"{suffix} OF "
        if text.startswith(prefix):
            base = text[len(prefix) :].strip()
            return f"{base}::{kind}" if base else None
        marker = f" {suffix}"
        if text.endswith(marker):
            base = text[: -len(marker)].strip()
            return f"{base}::{kind}" if base else None
    return None
...
def _load_cius_source_frame(source_kind: str, source_path: Path, member: str | None, *, table_key: str) -> pd.DataFrame:
    ...
    for col, norm in norm_map.items():
        ...
        elif norm.startswith("POPULATION"):
...
def parse_cius_local_rows(paths: RepoPaths, *, year: int) -> pd.DataFrame:
    ...
    def _normalize_output(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in output_cols:
            if col in {"official_count", "publication_population"}:
    ...
    for collection, spec in CIUS_TABLE_SPECS.items():
        ...
        for source_kind, source_path, member in sources:
            ...
            raw["publication_population"] = pd.to_numeric(
                raw.get("Population", pd.Series(pd.NA, index=raw.index)),
                errors="coerce",
            )
            value_cols = [col for col in CIUS_OFFENSE_COLUMNS.keys() if col in raw.columns]
            long = raw.melt(
                id_vars=list(spec["id_cols"]) + ["state_abbr", "publication_population"],
...
def match_cius_municipal_rows_to_jurisdictions(
    ...
    *,
    ...
    muni["publication_name_typed_std"] = muni["jurisdiction_name"].map(
        norm_typed_municipal_name
    )
    muni = muni[
        [
            "jurisdiction_id",
            ...
            "publication_name_exact_std",
            "publication_name_std",
            "publication_name_typed_std",
        ]
    ].drop_duplicates()

    municipal = municipal.reset_index(drop=True).copy()
    municipal["match_row_id"] = np.arange(len(municipal))
    municipal["publication_name_typed_std"] = municipal[
        "publication_name_exact_std"
    ].map(norm_typed_municipal_name)

    typed_candidates = muni[
        muni["publication_name_typed_std"].notna()
    ][
        ["jurisdiction_id", "state_abbr", "publication_name_typed_std"]
    ].drop_duplicates()
    typed_counts = (
        typed_candidates.groupby(
            ["state_abbr", "publication_name_typed_std"], dropna=False
        )["jurisdiction_id"]
        .nunique()
        .rename("repo_match_count")
        .reset_index()
    )
    typed_candidates = typed_candidates.merge(
        typed_counts,
        on=["state_abbr", "publication_name_typed_std"],
        how="left",
    )
    typed_unique = municipal.merge(
        typed_candidates[typed_candidates["repo_match_count"].eq(1)][
            ["jurisdiction_id", "state_abbr", "publication_name_typed_std"]
        ],
        on=["state_abbr", "publication_name_typed_std"],
        how="inner",
    ...
    current_alias_candidates = build_current_roster_municipal_alias_rows(
        agency_master=agency_master,
        agency_to_jurisdiction_crosswalk=agency_to_jurisdiction_crosswalk,
        jurisdictions=jurisdictions,
    ) if agency_master is not None and agency_to_jurisdiction_crosswalk is not None else pd.DataFrame()
    typed_ambiguous = municipal[
        municipal["publication_name_typed_std"].notna()
        & ~municipal["match_row_id"].isin(typed_unique["match_row_id"])
    ].copy()
    typed_roster = pd.DataFrame(columns=list(municipal.columns) + ["jurisdiction_id"])
    if not typed_ambiguous.empty and not current_alias_candidates.empty:
        typed_roster = typed_ambiguous.merge(
            current_alias_candidates,
            ...
            on=["jurisdiction_id", "state_abbr", "publication_name_typed_std"],
            how="inner",
        )
    ...
    untyped = municipal[municipal["publication_name_typed_std"].isna()].copy()
    exact = untyped.merge(
    ...
    resolved_ids = set(typed_unique["match_row_id"]) | set(
        typed_roster["match_row_id"]
    ) | set(exact["match_row_id"])
    unresolved = untyped[~untyped["match_row_id"].isin(resolved_ids)].copy()
    ...
    typed_alias = typed_ambiguous[
        ~typed_ambiguous["match_row_id"].isin(typed_roster["match_row_id"])
    ].copy()
    if not typed_alias.empty and not current_alias_candidates.empty:
        typed_alias = typed_alias.merge(
            current_alias_candidates,
            on=["state_abbr", "publication_name_exact_std"],
            how="inner",
        )
    else:
        typed_alias = pd.DataFrame(columns=list(municipal.columns) + ["jurisdiction_id"])

    # Pandas 2.x excludes all-null input columns when inferring concat dtypes and
    # warns that this will change. Removing those columns from each partial match
    # is value-preserving—the union concat restores them wherever another partial
    # match has data—and makes the schema stable under the future behavior.
    match_parts = [
        part.dropna(axis="columns", how="all")
        for part in [typed_unique, typed_roster, typed_alias, exact, fallback, agency_alias]
        if not part.empty
    ]
    matched = pd.concat(match_parts, ignore_index=True) if match_parts else pd.DataFrame()
    if matched.empty:
        return pd.DataFrame(columns=list(cius_rows.columns) + ["jurisdiction_id"])
    return matched.drop_duplicates(subset=["match_row_id"], keep="first").drop(columns=["match_row_id"])


def build_current_roster_municipal_alias_rows(
    *,
    agency_master: pd.DataFrame,
    agency_to_jurisdiction_crosswalk: pd.DataFrame,
    jurisdictions: pd.DataFrame,
) -> pd.DataFrame:
    """Current-roster municipal label -> one active municipal jurisdiction."""
    required = {"ori9", "roster_municipal_name_std", "source_presence_roster"}
    if not required.issubset(agency_master.columns):
        return pd.DataFrame(
            columns=["state_abbr", "publication_name_exact_std", "jurisdiction_id"]
        )
    municipal_ids = set(
        jurisdictions.loc[
            jurisdictions["jurisdiction_type"].eq("municipal"), "jurisdiction_id"
        ].astype(str)
    )
    ...
    crosswalk = crosswalk[
        crosswalk["jurisdiction_id"].astype(str).isin(municipal_ids)
        & crosswalk["weight"].gt(0.999)
    ].copy()
    crosswalk = crosswalk.groupby("ori9", dropna=False).filter(
        lambda group: group["jurisdiction_id"].nunique() == 1
    )
    aliases = agency_master[
        agency_master["source_presence_roster"].fillna(False).astype(bool)
        & agency_master["roster_municipal_name_std"].notna()
    ][["ori9", "state_abbr", "roster_municipal_name_std"]].merge(
        crosswalk[["ori9", "jurisdiction_id"]].drop_duplicates(),
        on="ori9",
        how="inner",
    )
    aliases = aliases.rename(
        columns={"roster_municipal_name_std": "publication_name_exact_std"}
    )
    counts = aliases.groupby(
        ["state_abbr", "publication_name_exact_std"], dropna=False
    )["jurisdiction_id"].transform("nunique")
    return aliases[counts.eq(1)][
        ["state_abbr", "publication_name_exact_std", "jurisdiction_id"]
