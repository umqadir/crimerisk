...
REQUIRED_INPUTS: list[dict[str, str]] = [
    ...
    {
        "stage": "Observations",
        "dataset": "FBI Return A master file (target year)",
        "expected_path": "data/FBI-UCR-Master-<year>/raw/reta-<year>.zip",
        "path_kind": "file",
        "required": "yes",
        "acquisition_type": "public download",
        "vintage": "target year",
        "consumed_by": "observations (srs_return_a_annual, years past the Kaplan restack)",
        "how_to_obtain": (
            f"Download the target year's Return A master file from the FBI Crime Data "
            f"Explorer: {CDE_DOWNLOADS}. Then run "
            "scripts/pull/parse_fbi_ucr_return_a_master.py --year <year>, which writes "
            "data/FBI-UCR-Return-A-Parsed-<year>-<stamp>/ and re-verifies the recovered "
            "field offsets against the published CIUS counts before writing anything."
        ),
        "notes": (
            "The Kaplan restack ends at 2024. Without this file the target year has one "
            "agency-level lane (the CIUS publication bundle) covering roughly half the "
            "national total, with no month field."
        ),
    },
    {
    ...
    {
        "stage": "Feature build",
        "dataset": "QCEW county annual-average employment (total and retail trade)",
        "expected_path": "data/qcew/raw/",
        "path_kind": "dir",
        "required": "no",
        "acquisition_type": "scripted public-data download",
        "vintage": "BLS QCEW annual averages, LODES vintage years and the target year",
        "consumed_by": "QCEW exposure updating (default-off exposure-normalizer lane)",
        "how_to_obtain": (
            "python scripts/pull/pull_qcew_county_employment.py — one CSV per (year, industry) "
            "from the QCEW open-data industry slice "
            "(https://data.bls.gov/cew/data/api/<year>/a/industry/<slug>.csv), industries 10 "
            "(total, all industries) and 44-45 (retail trade)."
        ),
        "notes": (
            "Required only when --enable-qcew-exposure-updating is set. Public domain (BLS). "
    ...
    # --- State publication extracts --------------------------------------------------
    {
        ...
        "dataset": "Connecticut DESPP town SRS annual extract",
        "expected_path": "data/CT-DESPP-2024/parsed/ct_despp_town_srs_2024.csv",
        "path_kind": "file",
        "required": "yes",
        "acquisition_type": "prepared public-data extract",
        "vintage": "2024",
        "consumed_by": "state publication source rows and CT town controls",
        "how_to_obtain": (
            "Export the 2024 DESPP SRS hierarchy-rule town table in list-format CSV "
            "from https://ct.beyond2020.com/ct_tops and run "
            "scripts/pull/parse_ct_despp_town_srs.py."
        ),
        "notes": "CTSP town identifiers are state-publication units, not FBI ORIs.",
    },
    {
        "stage": "State publications",
