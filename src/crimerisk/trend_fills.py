...
from crimerisk.level_lane import (
    NIBRS_TRANSITION_GUARD_YEARS,
    OWN_HISTORY_SUFFICIENT_CLEAN_YEARS,
    ...
    *,
    ...
    for year in range(int(year_start), int(year_end) + 1):
        preferred = build_agency_preferred_observations(
            paths=paths,
            year=year,
            reporting_year_end=int(year_end),
...
AGENCY_TARGET_ESTIMATE_COLUMNS = [
    ...
    "unresolved_mass",
    "history_effective_age",
    "history_effective_strength",
    "history_interval_sd",
]
...
def apply_masked_gap_reclassification(
    ...
    *,
    ...
    protected = out.get(
        "level_lane_hard_case_id", pd.Series(pd.NA, index=out.index, dtype="string")
    ).notna() | out.get(
        "level1_admission_status", pd.Series("", index=out.index, dtype="string")
    ...
    if "level_lane_original_count" not in out.columns:
        out["level_lane_original_count"] = pd.to_numeric(out["preferred_count"], errors="coerce")
    out.loc[ladder_rows, "preferred_count"] = 0.0
    ...
    out.loc[ladder_rows, "level1_admission_status"] = "coverage_defective"
    out.loc[ladder_rows, "level2_semantic_status"] = "semantically_unusable"
    out.loc[ladder_rows, "level_admission_reason"] = "masked_gap_coverage_contradiction"
    out.loc[ladder_rows, "level_repair_mode"] = "decayed_own_history_or_pooled"
    out.loc[partial_rows, "level1_admission_status"] = "valid_partial_lower_bound"
    out.loc[partial_rows, "level2_semantic_status"] = "definitionally_complete"
    out.loc[partial_rows, "level_admission_reason"] = "masked_gap_measured_partial_coverage"
    out.loc[partial_rows, "level_repair_mode"] = "partial_missing_increment"
    return out
...
def apply_stage1_adjudicated_usability(
    ...
    *,
    ...
    protected_structural_zero = (
        out.get(
            "level1_admission_status",
            pd.Series("", index=out.index, dtype="string"),
        ).astype("string").eq("corroborated_structural_zero")
    )
    protected_hard_case = out.get(
        "level_lane_hard_case_id", pd.Series(pd.NA, index=out.index, dtype="string")
    ).notna()
    ladder_rows = ladder_rows & ~protected_structural_zero & ~protected_hard_case
    partial_rows = partial_rows & ~protected_structural_zero & ~protected_hard_case
    ...
    out.loc[ladder_rows, "level1_admission_status"] = "coverage_defective"
    out.loc[ladder_rows, "level2_semantic_status"] = "semantically_unusable"
    out.loc[ladder_rows, "level_admission_reason"] = "stage1_adjudicated_reads_missing"
    out.loc[ladder_rows, "level_repair_mode"] = "decayed_own_history_or_pooled"
    out.loc[partial_rows, "level1_admission_status"] = "valid_partial_lower_bound"
    out.loc[partial_rows, "level2_semantic_status"] = "definitionally_complete"
    out.loc[partial_rows, "level_admission_reason"] = "stage1_adjudicated_partial_coverage"
    out.loc[partial_rows, "level_repair_mode"] = "partial_missing_increment"
...
# Calendar age is no longer an admissibility cliff. This compatibility name remains
# for identity/succession callers, but spans the complete retained history. The actual
# influence of history is continuous and governed by the frozen decay parameters below.
# Kept only as the identity/succession discontinuity lookback; the estimator does not
# use it as a history-admission cutoff.
FILL_MAX_REFERENCE_AGE_YEARS = 2
HISTORY_DECAY_HALF_LIFE_YEARS = 1.0
HISTORY_BASE_PSEUDOCOUNT_STRENGTH = 4.0
HISTORY_POOL_PSEUDOCOUNT_STRENGTH = 1.0

AGENCY_NO_HISTORY_ESTIMATE_SOURCE = "current_count_no_history"
AGENCY_EXPLICIT_SILENT_POOL_SOURCE = "pooled_silent_unit_explicit_footprint"
...
def _assert_no_fill_where_the_chosen_lane_reported(
    ...
    *,
    ...
    reason_coded_repair = (
        merged.get("level1_admission_status", pd.Series("", index=merged.index)).isin(
            {"coverage_defective", "source_identity_failure", "corroborated_structural_zero"}
        )
        | merged.get("level2_semantic_status", pd.Series("", index=merged.index)).isin(
            {
                "channel_omitted",
        ...
        | merged.get("level_repair_mode", pd.Series("", index=merged.index)).isin(
...
def _check_fill_mass_against_baseline(
    ...
    *,
    ...
    """Reason-coded replacement for the retired aggregate fill-mass ratchet.

    V2 intentionally admits more decayed-history repairs, so a global upper bound on fill
    mass would reject correct work. The blocking contract is instead row composition:
    every final agency estimate has exactly one reason-coded owner, and invalid fragments
    remain audit-only.
    """
    del paths  # retained in the call signature for compatibility with older callers
    if result.empty:
        return {"non_observed_estimate_mass": 0.0, "filled_agency_count": 0.0}
    components = result[["accepted_observed_mass", "repair_mass", "unresolved_mass"]].apply(
        pd.to_numeric, errors="coerce"
    ).fillna(0.0)
    estimated = pd.to_numeric(result["estimated_count"], errors="coerce").fillna(0.0)
    residual = estimated - components.sum(axis=1)
    bad = result[residual.abs().gt(1e-6)]
    if not bad.empty:
        raise ValueError(
            f"{len(bad)} agency estimates lack a single reason-coded mass owner: "
            + str(bad[["ori9", "offense", "agency_estimate_source", "level_admission_reason"]].head(20).to_dict(orient="records"))
        )
    invalid = result["level1_admission_status"].isin({"coverage_defective", "source_identity_failure"}) | result[
        "level2_semantic_status"
    ].isin({"channel_omitted", "wrong_category_mapping", "documented_swap", "semantically_unusable"})
    if components.loc[invalid, "accepted_observed_mass"].gt(1e-6).any():
        raise ValueError("invalid fragment mass entered the accepted-observed component")
...
def build_agency_target_estimates_from_panel(
    ...
    *,
    ...
    for column, default in (
        ...
        ("level_admission_reason", "legacy_supported"),
    ...
    if exclusion_keys:
        ...
        work["_reference_year_excluded"] = (
            work["_reference_year_excluded"].astype("boolean").fillna(False).astype(bool)
        )
    else:
        work["_reference_year_excluded"] = False
    work["_usable_as_reference"] = work["usable_as_observed"] & ~work["_reference_year_excluded"]

    # (c) An agency's pre-NIBRS-transition years describe a different collection. They
    # are dropped as fill references when enough post-transition clean years remain to
    # stand on; an agency that only just converted keeps them, because a discontinuous
    # reference is still better evidence than none at all.
    policy_switches = level_lane_policy()
    work["_pre_nibrs_transition_reference"] = False
    if (
        policy_switches.nibrs_transition_reference_guard
        and "level_nibrs_start_year" in work.columns
    ):
        start_year = pd.to_numeric(work["level_nibrs_start_year"], errors="coerce")
        row_year = pd.to_numeric(work["year"], errors="coerce")
        pre_transition = (
            start_year.notna()
            & row_year.lt(start_year + NIBRS_TRANSITION_GUARD_YEARS)
            & row_year.lt(int(target_year))
        )
        post_transition_years = (
            work.loc[
                work["_usable_as_reference"] & ~pre_transition & row_year.lt(int(target_year)),
                ["ori9", "offense", "year"],
            ]
            .groupby(["ori9", "offense"], dropna=False)["year"]
            .nunique()
        )
        enough = pd.MultiIndex.from_arrays(
            [work["ori9"], work["offense"]]
        ).map(post_transition_years).to_numpy()
        enough = pd.Series(enough, index=work.index).fillna(0.0).ge(2.0)
        work["_pre_nibrs_transition_reference"] = pre_transition & enough
        work["_usable_as_reference"] = (
            work["_usable_as_reference"] & ~work["_pre_nibrs_transition_reference"]
        )
    ...
    # Current-year complete reporters supply the pooled silent-unit rate. This is a
    # prior, not an agency observation, and is combined with own history below. Rows
    # admitted by the current build's level policy are valid publication values, but
    # they do not recalibrate repairs in that same build. Keeping the treated rows out
    # of the calibration pool prevents the admit/repair decision from moving an
    # already-adjudicated history repair (Jackson is the production sentinel).
    pool_rows = work[
        work["year"].eq(int(target_year))
        & work["usable_as_observed"]
        & work["level1_admission_status"].eq("valid_complete_year")
        & work["level2_semantic_status"].eq("definitionally_complete")
        & work["level_policy_resolution"].eq("not_applicable")
    ].copy()
    if "population" in pool_rows.columns:
        pool_rows["_population"] = pd.to_numeric(pool_rows["population"], errors="coerce").fillna(0.0)
    else:
        pool_rows["_population"] = 0.0
    pool_rows["_count"] = pd.to_numeric(pool_rows["preferred_count"], errors="coerce").fillna(0.0)
    pool_rows["_agency_type"] = pool_rows.get(
        "agency_type_norm", pd.Series("unknown", index=pool_rows.index)
    ).astype("string").fillna("unknown")
    pool_state_type = pool_rows.groupby(
        ["state_fips", "_agency_type", "offense"], dropna=False
    ).agg(y=("_count", "sum"), e=("_population", "sum"))
    pool_state_type["rate"] = pool_state_type["y"] / pool_state_type["e"].replace(0.0, np.nan)
    pool_state = pool_rows.groupby(["state_fips", "offense"], dropna=False).agg(y=("_count", "sum"), e=("_population", "sum"))
    pool_state["rate"] = pool_state["y"] / pool_state["e"].replace(0.0, np.nan)
    pool_national = pool_rows.groupby("offense", dropna=False).agg(y=("_count", "sum"), e=("_population", "sum"))
    pool_national["rate"] = pool_national["y"] / pool_national["e"].replace(0.0, np.nan)
    ...
    def _historical_estimate(
        *,
        ...
        population: float,
        agency_type: str,
        allow_no_history_pool: bool,
    ) -> tuple[float, str, float, float, float]:
        typed_rate = pool_state_type["rate"].get(
            (str(state_fips).zfill(2), str(agency_type), str(offense)), np.nan
        )
        state_rate = pool_state["rate"].get((str(state_fips).zfill(2), str(offense)), np.nan)
        national_rate = pool_national["rate"].get(str(offense), np.nan)
        rate = (
            float(typed_rate)
            if pd.notna(typed_rate)
            else float(state_rate)
            if pd.notna(state_rate)
            else float(national_rate)
            if pd.notna(national_rate)
            else np.nan
        )
        pool_mean = float(max(0.0, rate * population)) if np.isfinite(rate) and population > 0 else float(max(0.0, current_count))
        if usable_hist.empty:
            if allow_no_history_pool:
                return pool_mean, AGENCY_EXPLICIT_SILENT_POOL_SOURCE, np.nan, 0.0, float(np.sqrt(max(pool_mean, 0.0)))
            return float(max(0.0, current_count)), AGENCY_NO_HISTORY_ESTIMATE_SOURCE, np.nan, 0.0, np.nan
        # Hard regime gate: only the most recent continuous source/regime segment can
        # anchor the target. A change in either field is evidence of a discontinuity.
        ordered = usable_hist.sort_values("year").copy()
        latest = ordered.iloc[-1]
        same = pd.Series(True, index=ordered.index)
        for column in ("preferred_source", "reporting_regime"):
            if column in ordered.columns and pd.notna(latest.get(column)):
                same &= ordered[column].astype("string").eq(str(latest[column]))
        ordered = ordered.loc[same]
        if ordered.empty:
            if allow_no_history_pool:
                return pool_mean, AGENCY_EXPLICIT_SILENT_POOL_SOURCE, np.nan, 0.0, float(np.sqrt(max(pool_mean, 0.0)))
            return float(max(0.0, current_count)), AGENCY_NO_HISTORY_ESTIMATE_SOURCE, np.nan, 0.0, np.nan
        ages = int(target_year) - pd.to_numeric(ordered["year"], errors="coerce").to_numpy(dtype=float)
        weights = np.exp(-np.log(2.0) * ages / float(HISTORY_DECAY_HALF_LIFE_YEARS))
        scaled = []
        for hist_row in ordered.itertuples(index=False):
            raw_value = pd.to_numeric(getattr(hist_row, "preferred_count"), errors="coerce")
            raw = float(max(0.0, float(raw_value) if pd.notna(raw_value) else 0.0))
            audit = trend_fill_lookup.resolve(state_fips=state_fips, offense=offense, base_year=int(getattr(hist_row, "year")))
            scaled.append(raw * float(audit.ratio))
        scaled_values = np.asarray(scaled, dtype=float)
        own_mean = float(np.average(scaled_values, weights=weights))
        effective_strength = float(HISTORY_BASE_PSEUDOCOUNT_STRENGTH * weights.sum())
        pool_mean = float(max(0.0, rate * population)) if np.isfinite(rate) and population > 0 else own_mean
        # (d) An agency with a long clean run of its own is not better described by the
        # rate of agencies like it. Past five clean years the pooled pseudocount is
        # dropped rather than merely outweighed: Jacksonville and Kansas City each have
        # a decade of steady filings, and the pooled prior only pulls them toward a
        # state-and-type average they have never been at.
        clean_years = int(pd.to_numeric(ordered["year"], errors="coerce").nunique())
        if (
            policy_switches.prefer_own_history_over_pool
            and clean_years >= OWN_HISTORY_SUFFICIENT_CLEAN_YEARS
        ):
            estimate = own_mean
        else:
            estimate = (
                effective_strength * own_mean
                + HISTORY_POOL_PSEUDOCOUNT_STRENGTH * pool_mean
            ) / (effective_strength + HISTORY_POOL_PSEUDOCOUNT_STRENGTH)
        effective_age = float(np.average(ages, weights=weights))
        between = float(np.average((scaled_values - own_mean) ** 2, weights=weights)) if len(scaled_values) > 1 else own_mean
        interval_sd = float(np.sqrt(max(estimate, 0.0) + between + effective_age * max(estimate, 0.0) / HISTORY_DECAY_HALF_LIFE_YEARS))
        return float(max(0.0, estimate)), "decayed_history_hierarchical", effective_age, effective_strength, interval_sd
    ...
    for (ori9, offense), grp in work.groupby(["ori9", "offense"], sort=False):
        ...
        agency_type = str(grp["agency_type_norm"].dropna().iloc[0]) if "agency_type_norm" in grp.columns and grp["agency_type_norm"].notna().any() else "unknown"
        ...
        def _current_text(column: str, default: str) -> str:
            if current.empty or column not in current.columns or pd.isna(current[column].iloc[0]):
                return default
            return str(current[column].iloc[0])

        level1_status = _current_text("level1_admission_status", "coverage_defective")
        level2_status = _current_text("level2_semantic_status", "semantically_unusable")
        level_reason = _current_text("level_admission_reason", "no_current_row")
        repair_mode = _current_text("level_repair_mode", "pooled_silent_unit")
        external_check_status = _current_text("external_check_status", "unavailable")
        if repair_mode == "soft_benchmark_reconciliation":
            # An explicit registry refusal of agency-level repair is owned by the
            # benchmark lane. Do not let otherwise usable history double-own it.
            usable_hist = usable_hist.iloc[0:0]
        original_count_value = pd.to_numeric(current.get("level_lane_original_count", pd.Series([current_count])).iloc[0], errors="coerce") if not current.empty else 0.0
        original_count = float(original_count_value) if pd.notna(original_count_value) else 0.0
        replacement_value = pd.to_numeric(
            current.get("level_lane_replacement_count", pd.Series([np.nan])).iloc[0],
            errors="coerce",
        ) if not current.empty else np.nan
        if current.empty:
            target_population = pd.to_numeric(
                grp.get("_target_population", pd.Series(0.0, index=grp.index)),
                errors="coerce",
            ).dropna()
            target_population_values = target_population.unique()
            if len(target_population_values) > 1:
                raise ValueError(f"Conflicting target populations for {ori9}")
            current_population = (
                float(target_population_values[0]) if len(target_population_values) else 0.0
            )
            allow_no_history_pool = bool(
                grp.get(
                    "_explicit_target_population_eligible",
                    pd.Series(False, index=grp.index),
                )
                .astype("boolean")
                .fillna(False)
                .astype(bool)
            ...
            population_value = pd.to_numeric(
                current.get("population", pd.Series([0.0])).iloc[0], errors="coerce"
            )
            current_population = float(population_value) if pd.notna(population_value) else 0.0
            allow_no_history_pool = bool(
                current.get("_explicit_silent_pool", pd.Series([False]))
                .fillna(False)
                .astype(bool)
                .iloc[0]
            )
        if repair_mode == "soft_benchmark_reconciliation":
            allow_no_history_pool = False
        history_effective_age = np.nan
        history_effective_strength = 0.0
        history_interval_sd = np.nan
        if level1_status == "corroborated_structural_zero":
            estimated = 0.0
            source = "corroborated_structural_zero"
        elif repair_mode == "covered_by_covering_agency":
            estimated = 0.0
            source = "covered_by_covering_agency"
        elif repair_mode in {"source_supported_replacement", "swap_reclassification"} and pd.notna(replacement_value):
            estimated = float(max(0.0, replacement_value))
            source = repair_mode
        elif not current.empty and current_usable:
            ...
            if 0 < months < 12 and current_count <= 0:
                ...
                if agency_sanity_cap_flag:
                    cap_estimate, cap_source, history_effective_age, history_effective_strength, history_interval_sd = _historical_estimate(
                        ...
                        allow_no_history_pool=allow_no_history_pool,
                    )
                    partial_uplift_cap_estimate = float(max(0.0, cap_estimate))
                    estimated = max(current_count, partial_uplift_cap_estimate)
        ...
        if not np.isfinite(estimated):
            invalid_fragment = level1_status in {"coverage_defective", "source_identity_failure"} or level2_status in {"channel_omitted", "wrong_category_mapping", "semantically_unusable"}
            estimate_floor = 0.0 if invalid_fragment else current_count
            estimated, source, history_effective_age, history_effective_strength, history_interval_sd = _historical_estimate(
                usable_hist=usable_hist,
                state_fips=state_fips,
                offense=str(offense),
                current_count=estimate_floor,
                population=current_population,
                agency_type=agency_type,
                allow_no_history_pool=allow_no_history_pool,
            )
        ...
        if repair_mode in {"source_supported_replacement", "swap_reclassification"}:
            source = repair_mode
        rows.append(
            {
                "ori9": str(ori9),
                ...
                "agency_adjustment_count": float(max(0.0, estimated - (0.0 if level1_status in {"coverage_defective", "source_identity_failure"} or level2_status in {"channel_omitted", "wrong_category_mapping", "semantically_unusable"} else current_count))),
                ...
                "level1_admission_status": level1_status,
                "level2_semantic_status": level2_status,
                "level_admission_reason": level_reason,
                "level_repair_mode": repair_mode,
                "external_check_status": external_check_status,
                "accepted_observed_mass": float(current_count if level1_status in {"valid_complete_year", "valid_partial_lower_bound"} and level2_status == "definitionally_complete" else 0.0),
                "repair_mass": float(
                    0.0
                    if level1_status == "unresolved_review"
                    else estimated
                    if repair_mode in {"source_supported_replacement", "swap_reclassification"}
                    else max(
                        0.0,
                        ...
                        - (
                            ...
                            in {
                                "valid_complete_year",
                                "valid_partial_lower_bound",
                                "corroborated_structural_zero",
                            }
                            and level2_status == "definitionally_complete"
                            else 0.0
                        ),
                    )
                ),
                "invalid_fragment_audit_mass": float(original_count if level1_status in {"coverage_defective", "source_identity_failure"} or level2_status in {"channel_omitted", "wrong_category_mapping", "documented_swap", "semantically_unusable"} else 0.0),
                "unresolved_mass": float(estimated if level1_status == "unresolved_review" else 0.0),
                "history_effective_age": history_effective_age,
                "history_effective_strength": history_effective_strength,
                "history_interval_sd": history_interval_sd,
            }
        )
    result = pd.DataFrame(rows, columns=columns)
    # Canonical pre-overlap/pre-jurisdiction-aggregation boundary: a review
    # hold publishes the admitted input and nothing else. Link weighting, CT
    # overlap residualization, and benchmark reconciliation occur later; a
    # partial-year annualization or history estimate must carry another mode.
    held = result["level_repair_mode"].astype("string").eq("unchanged_review_hold")
    held_delta = (
        pd.to_numeric(result.loc[held, "estimated_count"], errors="coerce")
        - pd.to_numeric(result.loc[held, "reported_count_current"], errors="coerce")
    )
    if held_delta.abs().gt(1e-9).any():
        bad_index = held_delta.index[held_delta.abs().gt(1e-9)]
        bad = result.loc[
            bad_index,
            [
                "ori9",
        ...
        raise ValueError(
            f"{len(bad)} unchanged review hold(s) changed the admitted input at "
            "the pre-overlap/pre-jurisdiction-aggregation sizing boundary: "
...
def build_agency_allocation_target_estimates(
    *,
    ...
    agency_panel = agency_panel.copy()
    agency_panel["_explicit_silent_pool"] = agency_panel.get(
        "level_admission_reason", pd.Series(pd.NA, index=agency_panel.index)
    ).astype("string").eq("uncorroborated_near_zero_filed_vector")
    master_path = paths.state_dir / "reference" / "agency_master.parquet"
    if master_path.exists() and (
        "agency_type_norm" not in agency_panel.columns
        or "population_latest_nibrs" not in agency_panel.columns
    ):
        master = pd.read_parquet(master_path)
        for column in ("agency_type_norm", "population_latest_nibrs"):
            if column not in master.columns:
                master[column] = pd.NA
        master = master[
            ["ori9", "agency_type_norm", "population_latest_nibrs"]
        ].drop_duplicates("ori9")
        missing_master_columns = [
            column
            for column in ("agency_type_norm", "population_latest_nibrs")
            if column not in agency_panel.columns
        ]
        agency_panel = agency_panel.merge(
            master[["ori9", *missing_master_columns]], on="ori9", how="left"
        )
    if "population" not in agency_panel.columns:
        population_source = (
            agency_panel["population_latest_nibrs"]
            if "population_latest_nibrs" in agency_panel.columns
            else pd.Series(0.0, index=agency_panel.index)
        )
        agency_panel["population"] = pd.to_numeric(population_source, errors="coerce").fillna(0.0)
    # FBI headers commonly report zero population for tribal departments even
    # when an explicit reservation footprint is configured.  Size silent-unit
    # priors from that exclusive footprint instead of returning a synthetic zero.
    agency_panel["_target_population"] = 0.0
    agency_panel["_explicit_target_population_eligible"] = False
    footprint_population = load_custom_footprint_population_by_target(paths)
    if not footprint_population.empty:
        agency_panel = agency_panel.merge(footprint_population, on="ori9", how="left")
        agency_panel["_explicit_silent_pool"] = (
            agency_panel["_explicit_silent_pool"].fillna(False).astype(bool)
            | agency_panel["_covered_population"].notna()
        )
        population = pd.to_numeric(agency_panel["population"], errors="coerce").fillna(0.0)
        agency_panel["population"] = population.where(
            population.gt(0.0),
            pd.to_numeric(agency_panel["_covered_population"], errors="coerce").fillna(0.0),
        )
        owned_population = pd.to_numeric(
            agency_panel["_covered_population"], errors="coerce"
        )
        target_rows = pd.to_numeric(agency_panel["year"], errors="coerce").eq(int(year))
        current_population = (
            agency_panel.loc[target_rows, ["ori9", "population"]]
            .assign(population=lambda d: pd.to_numeric(d["population"], errors="coerce"))
            .groupby("ori9")["population"]
            .agg(["min", "max"])
        )
        conflicting = current_population[
            ~np.isclose(current_population["min"], current_population["max"], atol=1e-9)
        ]
        if not conflicting.empty:
            raise ValueError(
                "Conflicting target-year populations for custom-footprint agencies: "
                f"{conflicting.index.astype(str).tolist()[:20]}"
            )
        has_current = agency_panel["ori9"].isin(current_population.index)
        agency_panel["_target_population"] = agency_panel["ori9"].map(
            current_population["max"]
        )
        agency_panel.loc[~has_current, "_target_population"] = owned_population.loc[
            ~has_current
        ]
        agency_panel["_target_population"] = pd.to_numeric(
            agency_panel["_target_population"], errors="coerce"
        ).fillna(0.0)
        agency_panel["_explicit_target_population_eligible"] = (
            has_current & agency_panel["_explicit_silent_pool"]
        ) | (~has_current & owned_population.notna())
        agency_panel = agency_panel.drop(columns=["_covered_population"])
    if "level1_admission_status" not in agency_panel.columns:
        # Allocation calls this shared wrapper directly, while controls supplies an
        # already-admitted panel. Apply the identical admission/masked-gap/legacy
        # adjudication sequence here so every consumer sizes an agency-offense from
        # the same reason-coded ledger without changing allocation.py.
        agency_panel = apply_level_lane_admission(
            agency_panel, paths=paths, target_year=int(year)
        ).panel
        if masked_gap_flags is None:
            masked_gap_flags = build_masked_gap_flags(
                paths, target_year=int(year), agency_panel=agency_panel
            )
        agency_panel = apply_masked_gap_reclassification(
            agency_panel,
        ...
        if stage1_adjudications is None:
            stage1_adjudications = build_usability_directives(
                paths, target_year=int(year)
            )
        agency_panel, _legacy_counts = apply_stage1_adjudicated_usability(
            agency_panel,
            target_year=int(year),
            directives=stage1_adjudications,
        )
    ...
    print(
        ...
        f"{counts.get('succession_explicit_oris_added', 0)} explicit reporter migrations",
        flush=True,
    )
    return estimates
