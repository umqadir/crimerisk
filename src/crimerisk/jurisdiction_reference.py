...
from crimerisk.agency_identity import apply_live_successor_footprint_rule
...
def _infer_special_bucket(text_candidates: list[str], agency_type_norm: str | None) -> tuple[str, str | None, str | None]:
    ...
    statewide_public_safety = (
        " DEPARTMENT OF PUBLIC SAFETY" in text
        and " CITY OF " not in text
        and " COUNTY " not in text
    )
    if statewide_public_safety or any(keyword in text for keyword in _STATEWIDE_KEYWORDS):
        return "statewide_overlap", None, "statewide"
    # An agency class is not a footprint.  In particular, the FBI roster's
    # `Other State Agency` and `Other` classes contain local hospitals, capitol-police
    # field offices, conservation departments, constables, and other bounded authorities.
    # Calling an unresolved row "statewide" makes the overlap allocator paint its mass
    # across every block group in the state and defeats the unlocated-mass safeguard.
    # Only an affirmative name/registry witness above is allowed to declare statewide
    # geometry; everything else stays unresolved until a supported local footprint exists.
    return "localized_special_overlap", None, None
...
def _resolve_unassigned_municipal(
    ...
    *,
    ...
    cousub_town_states = {"CT", "ME", "MA", "NH", "NY", "RI", "VT", "WI"}
    for text in candidates:
        township_like = text.endswith(" TOWNSHIP") or text.endswith(
            " CHARTER TOWNSHIP"
        )
        town_like = text.endswith(" TOWN") and state_abbr in cousub_town_states
        if township_like or town_like:
...
def _build_unassigned_fallbacks(
    *,
    ...
    for row in unassigned.to_dict(orient="records"):
        ...
        if is_local_like and special_subtype is None:
            local_rows.append(
                {
                    "ori9": row.get("ori9"),
                    "ori7": row.get("ori7"),
...
def _reclassify_nongovernment_census_geographies(
    full_local: pd.DataFrame,
) -> pd.DataFrame:
    """Prevent unreviewed statistical Census units from becoming police jurisdictions.

    A CDP is not a government, so an automatic agency-seat/name match may never create
    a patrol jurisdiction from one.  A reviewed local-resolution or geometry override
    may, however, use the Census polygon as the best available boundary for a real
    police service area (for example Ocean Pines or Terre du Lac).
    """
    out = full_local.copy()
    municipal = out["final_decision"].isin(["municipal_place", "municipal_cousub"])
    label = out["resolved_label"].astype("string").str.strip().str.casefold()
    nongovernment = label.str.endswith((" cdp", " ccd", " precinct"), na=False)
    explicit_surrogate = out["resolution_source"].astype("string").isin(
        ["municipal_geometry_override", "local_resolution_override"]
    )
    rejected = municipal & nongovernment & ~explicit_surrogate
    if rejected.any():
        out.loc[rejected, "final_decision"] = "reclassify_nonmunicipal"
        out.loc[rejected, ["resolved_geo_type", "resolved_geoid", "resolved_label"]] = None
        out.loc[rejected, "manual_review_flag"] = True
        out.loc[rejected, "resolution_source"] = "nongovernment_census_unit_rejected"
        out.loc[rejected, "has_final_municipal_geoid"] = False
        out.loc[rejected, "match_method"] = "nongovernment_census_unit_rejected"
    return out


def _assert_no_nongovernment_municipal_jurisdictions(master: pd.DataFrame) -> None:
    municipal = master[master["jurisdiction_type"].astype("string").eq("municipal")]
    label = municipal["jurisdiction_name"].astype("string").str.strip().str.casefold()
    reviewed_surrogate = municipal["manual_review_flag"].fillna(False).astype(bool)
    bad = municipal[
        label.str.endswith((" cdp", " ccd", " precinct"), na=False)
        & ~reviewed_surrogate
        & ~municipal["jurisdiction_name"].astype("string").eq(
            "North Slope census subarea"
        )
    ]
    if not bad.empty:
        raise ValueError(
            "Non-government Census units cannot be municipal jurisdictions: "
            + str(
                bad[["jurisdiction_id", "jurisdiction_name"]]
                .head(20)
...
def _assert_no_institutional_municipal_exclusives(
    crosswalk: pd.DataFrame,
    agency_master: pd.DataFrame,
    jurisdiction_master: pd.DataFrame | None = None,
) -> None:
    """Reject school/campus/special-purpose agencies from the municipal lane."""
    municipal = crosswalk[
        crosswalk["jurisdiction_id"].astype("string").str.contains(
            ":municipal:", regex=False, na=False
        )
        & crosswalk["relationship_type"].astype("string").eq("exclusive")
    ].copy()
    if municipal.empty:
        return
    agencies = agency_master[
        ["ori9", "agency_name_raw", "agency_type_norm"]
    ].drop_duplicates("ori9")
    municipal = municipal.merge(
        agencies, left_on="ori", right_on="ori9", how="left"
    )
    if jurisdiction_master is not None:
        municipal = municipal.merge(
            jurisdiction_master[["jurisdiction_id", "jurisdiction_name"]],
            on="jurisdiction_id",
            how="left",
        )
    else:
        municipal["jurisdiction_name"] = pd.NA

    def _agency_government_name(value: object) -> str:
        text = _std_text(value) or ""
        for suffix in (" POLICE DEPARTMENT", " POLICE DEPT", " POLICE", " PD"):
            if text.endswith(suffix):
                text = text[: -len(suffix)].strip()
        return text

    def _jurisdiction_government_name(value: object) -> str:
        text = _std_text(value) or ""
        for suffix in (" BOROUGH", " CITY", " TOWN", " VILLAGE"):
            if text.endswith(suffix):
                text = text[: -len(suffix)].strip()
        return text

    agency_name = municipal["agency_name_raw"].map(_agency_government_name)
    jurisdiction_name = municipal["jurisdiction_name"].map(
        _jurisdiction_government_name
    )
    institutional_words = municipal["agency_name_raw"].astype("string").str.contains(
        r"\b(?:college|university|school|schools)\b",
        case=False,
        regex=True,
        na=False,
    )
    institutional_name = institutional_words & agency_name.ne(jurisdiction_name)
    special_type = municipal["agency_type_norm"].astype("string").isin(
        ["special_jurisdiction", "state_law_enforcement"]
    )
    bad = municipal[institutional_name | special_type]
    if not bad.empty:
        raise ValueError(
            "Institutional or special-purpose agencies cannot be exclusive municipal "
            "reporters: "
            + str(
                bad[
                    [
...
def _apply_municipal_geometry_overrides(
    ...
    *,
    ...
    # The reviewed registry also documents AK/HI and territory resolutions, but
    # this product deliberately has no geometry for those states.  Ignore those
    # rows here while retaining the fail-closed target check for every in-scope
    # override.
    in_scope = ~production_scope_excluded(merged["state_abbr"])
    active_override = has_override & in_scope
    mapped = active_override & merged["override_decision"].isin(["map_to_place", "map_to_cousub"])
    if mapped.any():
    ...
    excluded = active_override & merged["override_decision"].eq("exclude")
    if excluded.any():
...
def build_jurisdiction_master(
    *,
    full_local: pd.DataFrame,
    full_nonlocal: pd.DataFrame,
    ct_town_registry: pd.DataFrame | None = None,
) -> pd.DataFrame:
    ...
    municipal = pd.concat(
        [
            ...
            ct_town_jurisdiction_rows(ct_town_registry)
            if ct_town_registry is not None
            else pd.DataFrame(),
        ],
        ignore_index=True,
    )
...
def build_agency_to_jurisdiction_crosswalk(
    *,
    ...
    ct_crosswalk = (
        ct_town_crosswalk_rows(ct_town_registry)
        if ct_town_registry is not None
        else pd.DataFrame()
    )
    # A Connecticut town publication is footprint-complete for its named town.
    # Do not sum a same-town FBI rendering into that numerator: submission
    # completeness and polygon coverage are different contracts.
    if not ct_crosswalk.empty:
        ct_owned_jurisdictions = set(ct_crosswalk["jurisdiction_id"].astype(str))
        local_crosswalk = local_crosswalk.loc[
            ~local_crosswalk["jurisdiction_id"].astype("string").isin(
                ct_owned_jurisdictions
            )
        ].copy()
        nonlocal_crosswalk = nonlocal_crosswalk.loc[
            ~nonlocal_crosswalk["jurisdiction_id"].astype("string").isin(
                ct_owned_jurisdictions
            )
        ].copy()
    out = pd.concat(
        [local_crosswalk, nonlocal_crosswalk, ct_crosswalk], ignore_index=True
    )
...
def build_reference_artifacts(
    *,
    ...
    full_local = _reclassify_nongovernment_census_geographies(full_local).sort_values(
        "ori9"
    ).reset_index(drop=True)
    full_local = apply_live_successor_footprint_rule(
        full_local,
        agency_master=agency_master,
    ...
    ct_town_registry = load_ct_town_coverage_registry(get_paths())
    tiger = _load_all_tiger_lookups(paths=get_paths()).copy()
    valid_ct_cousubs = set(
        tiger[
            tiger["state_fips"].astype("string").str.zfill(2).eq("09")
            & tiger["geo_type"].astype("string").eq("cousub")
        ]["geoid"].astype("string")
    )
    unknown_ct_geoids = sorted(
        set(ct_town_registry["cousub_geoid_2020"].astype(str)) - valid_ct_cousubs
    )
    if unknown_ct_geoids:
        raise ValueError(
            "CT town registry GEOIDs absent from the 2020 TIGER county-subdivision "
            f"lookup: {unknown_ct_geoids[:20]}"
        )
    jurisdiction_master = build_jurisdiction_master(
        full_local=full_local,
        full_nonlocal=full_nonlocal,
        ct_town_registry=ct_town_registry,
    )
    _assert_no_nongovernment_municipal_jurisdictions(jurisdiction_master)
    ...
    _assert_no_institutional_municipal_exclusives(
        crosswalk, agency_master, jurisdiction_master
    )
