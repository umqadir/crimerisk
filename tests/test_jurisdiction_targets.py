...
import crimerisk.trend_fills as trend_fills
...
from crimerisk.jurisdiction_targets import (
    ...
    assert_computed_current_year_fills_consumed,
...
def _target_population_panel(
    *, include_current: bool = False, target_ori: str = "AA0000001"
) -> pd.DataFrame:
    rows = [
        (target_ori, TARGET_YEAR - 1, "larceny", 10.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
        ("AA0000002", TARGET_YEAR, "larceny", 100.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
    ]
    if include_current:
        rows.append(
            (
    ...
    panel["population"] = panel["ori9"].map({target_ori: 200.0, "AA0000002": 1_000.0})
    panel["agency_type_norm"] = "local_police"
    panel["level1_admission_status"] = "valid_complete_year"
    panel["level2_semantic_status"] = "definitionally_complete"
    panel["level_repair_mode"] = "none"
    panel["level_policy_resolution"] = "not_applicable"
    if include_current:
        repair = panel["ori9"].eq(target_ori) & panel["year"].eq(TARGET_YEAR)
        panel.loc[repair, "level1_admission_status"] = "coverage_defective"
        panel.loc[repair, "level2_semantic_status"] = "semantically_unusable"
        panel.loc[repair, "level_repair_mode"] = "decayed_own_history"
    return panel


def test_absent_current_row_uses_reviewed_target_population_in_history_prior() -> None:
    panel = _target_population_panel()
    target = panel["ori9"].eq("AA0000001")
    panel.loc[target, "_target_population"] = 300.0
    panel.loc[target, "_explicit_target_population_eligible"] = True
    ...
    estimate = estimates[estimates["ori9"].eq("AA0000001")].iloc[0]

    assert estimate["agency_estimate_source"] == "decayed_history_hierarchical"
    assert float(estimate["history_effective_strength"]) == pytest.approx(2.0)
    assert float(estimate["estimated_count"]) == pytest.approx((2.0 * 10.0 + 30.0) / 3.0)


def test_absent_current_row_without_reviewed_target_population_keeps_history_only() -> None:
    panel = _target_population_panel()
    estimates = build_agency_target_estimates_from_panel(
        panel,
        target_year=TARGET_YEAR,
    ...
    estimate = estimates[estimates["ori9"].eq("AA0000001")].iloc[0]
    assert float(estimate["estimated_count"]) == pytest.approx(10.0)


def test_present_current_row_keeps_its_population_authoritative() -> None:
    panel = _target_population_panel(include_current=True)
    target = panel["ori9"].eq("AA0000001")
    panel.loc[target, "_target_population"] = 999.0
    panel.loc[target, "_explicit_target_population_eligible"] = True
    ...
    estimate = estimates[estimates["ori9"].eq("AA0000001")].iloc[0]
    assert float(estimate["estimated_count"]) == pytest.approx((2.0 * 10.0 + 20.0) / 3.0)


def test_absent_current_row_without_usable_history_can_use_reviewed_pool() -> None:
    panel = _target_population_panel()
    target = panel["ori9"].eq("AA0000001")
    panel.loc[target, "usable_as_observed"] = False
    panel.loc[target, "_target_population"] = 300.0
    panel.loc[target, "_explicit_target_population_eligible"] = True
    ...
    estimate = estimates[estimates["ori9"].eq("AA0000001")].iloc[0]
    assert estimate["agency_estimate_source"] == "pooled_silent_unit_explicit_footprint"
    assert float(estimate["estimated_count"]) == pytest.approx(30.0)


def test_absent_current_row_rejects_conflicting_target_population_metadata() -> None:
    panel = _target_population_panel()
    duplicate = panel[panel["ori9"].eq("AA0000001")].copy()
    duplicate["year"] = TARGET_YEAR - 2
    panel = pd.concat([panel, duplicate], ignore_index=True)
    target = panel["ori9"].eq("AA0000001")
    panel.loc[target, "_explicit_target_population_eligible"] = True
    panel.loc[target, "_target_population"] = [300.0, 301.0]

    with pytest.raises(ValueError, match="Conflicting target populations"):
        build_agency_target_estimates_from_panel(
            panel,
            target_year=TARGET_YEAR,
            trend_fill_lookup=TrendFillLookup(state_map={}, national_map={}),
...
def test_wrapper_broadcasts_reviewed_population_to_an_absent_current_row(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_ori = "AZ0018900"
    panel = _target_population_panel(target_ori=target_ori)
    paths = SimpleNamespace(repo_root=tmp_path, state_dir=tmp_path / "state")

    def estimate(population: float) -> float:
        monkeypatch.setattr(
            trend_fills,
            "load_custom_footprint_population_by_target",
            lambda _paths: pd.DataFrame(
                {"ori9": [target_ori], "_covered_population": [population]}
            ),
        )
        estimates = build_agency_allocation_target_estimates(
            ...
            stage1_adjudications=pd.DataFrame(),
            reference_year_exclusions=pd.DataFrame(),
            succession_ledger=pd.DataFrame(),
        )
        return float(
            estimates.loc[estimates["ori9"].eq(target_ori), "estimated_count"].iloc[0]
        )

    old = estimate(94_511.0)
    new = estimate(163_545.0)
    assert old == pytest.approx((2.0 * 10.0 + 0.1 * 94_511.0) / 3.0)
    assert new == pytest.approx((2.0 * 10.0 + 0.1 * 163_545.0) / 3.0)
    assert new - old == pytest.approx(0.1 * (163_545.0 - 94_511.0) / 3.0)
...
def _write_state_tree(
    ...
    *,
    ...
    shutil.copy2(
        get_paths().repo_root / "configs" / "federal_ori_scope.csv",
...
def test_dc_overlap_excludes_fbi_hq_and_retains_fixed_footprint_controls(tmp_path):
    jurisdiction_id = "11:statewide_overlap_layer"
    crosswalk = pd.DataFrame(
        {
            "ori": ["DCFBIWA00", "DC0019000", "DCMTP0000"],
            "state_fips": ["11", "11", "11"],
            "state_abbr": ["DC", "DC", "DC"],
            "jurisdiction_id": [jurisdiction_id] * 3,
            "relationship_type": ["overlap"] * 3,
            "weight": [1.0] * 3,
            "overlap_subtype": [pd.NA] * 3,
        }
    )
    paths = _write_state_tree(
        tmp_path,
        jurisdictions=_jurisdictions(
            [(jurisdiction_id, "statewide_overlap_layer", "11", "DC")]
        ),
        crosswalk=crosswalk,
        block_groups=_block_groups([]),
        agency_master=pd.DataFrame(
            {
                "ori9": ["DCFBIWA00", "DC0019000", "DCMTP0000"],
                "agency_name_std": [
                    "FEDERAL BUREAU OF INVESTIGATION",
                    "ARCHITECT OF THE CAPITOL",
                    "METRO TRANSIT POLICE",
                ],
                "agency_type_raw": ["federal", "federal", "transit"],
            }
        ),
    )
    panel = _panel_rows(
        [
            ("DCFBIWA00", TARGET_YEAR, "murder", 41.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
            ("DC0019000", TARGET_YEAR, "rape", 4.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
            ("DCMTP0000", TARGET_YEAR, "robbery", 301.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
        ]
    )
    panel["state_fips"] = "11"
    panel["state_abbr"] = "DC"
    estimates = _estimate_rows(
        [
            ("DCFBIWA00", "murder", 41.0, 41.0, "observed"),
            ("DC0019000", "rape", 4.0, 4.0, "observed"),
            ("DCMTP0000", "robbery", 301.0, 301.0, "observed"),
        ]
    )
    estimates["state_fips"] = "11"

    out = build_jurisdiction_year_estimates(
        paths=paths,
        config=JurisdictionTargetConfig(year_start=TARGET_YEAR, target_year=TARGET_YEAR),
        agency_panel=panel,
        agency_estimates=estimates,
    )
    amounts = out.set_index("offense")["estimated_count"]
    assert float(amounts["murder"]) == 0.0
    assert float(amounts["rape"]) == 4.0
    assert float(amounts["robbery"]) == 301.0
...
def test_a_computed_fill_must_be_consumed_or_explicitly_refused():
    crosswalk = _crosswalk([("A", "28", "j1", 1.0)]).rename(columns={"ori": "ori9"})
    estimates = _estimate_rows([("A", "larceny", 0.0, 25.0, "hist_median")])
    dropped = pd.DataFrame(
        {
            "jurisdiction_id": ["j1"],
            "offense": ["larceny"],
            "current_year_fill_count": [0.0],
        }
    )
    with pytest.raises(ValueError, match="computed current-year fill"):
        assert_computed_current_year_fills_consumed(
            agency_estimates=estimates, crosswalk=crosswalk, targets=dropped
        )
    consumed = dropped.assign(current_year_fill_count=25.0)
    assert_computed_current_year_fills_consumed(
        agency_estimates=estimates, crosswalk=crosswalk, targets=consumed
    )
    empty_refusal = dropped.assign(current_year_fill_refusal_reason="  ")
    with pytest.raises(ValueError, match="computed current-year fill"):
        assert_computed_current_year_fills_consumed(
            agency_estimates=estimates, crosswalk=crosswalk, targets=empty_refusal
        )
    refused = dropped.assign(current_year_fill_refusal_reason="superseded source contract")
    assert_computed_current_year_fills_consumed(
        agency_estimates=estimates, crosswalk=crosswalk, targets=refused
    )
...
def test_jackson_ms_takes_no_pre_imputation_jurisdiction_fill(tmp_path):
    """GOLDEN. V2 retains old clean history with exponential decay instead of an age cliff."""
    ...
    jackson_estimate = estimates[estimates["ori9"].eq(JACKSON_ORI)]
    assert len(jackson_estimate) == 1
    assert jackson_estimate["agency_estimate_source"].iloc[0] == "decayed_history_hierarchical"
    assert 0.0 < float(jackson_estimate["history_effective_strength"].iloc[0]) < 1.0
    ...
    expected = float(jackson_estimate["estimated_count"].iloc[0])
    assert float(jackson["estimated_count"].iloc[0]) == pytest.approx(expected)
    assert float(jackson["current_year_fill_count"].iloc[0]) == pytest.approx(expected)
    assert jackson["estimate_source"].iloc[0] == "agency_rollup_fill"


def test_level_policy_admissions_do_not_recalibrate_existing_history_repairs():
    """A same-build admission is publishable evidence, not a peer-pool calibrator."""
    repair_ori = "MS0000001"
    peer_ori = "MS0000002"
    treated_ori = "MS0000003"
    panel = _panel_rows(
        [
            (repair_ori, 2023, "larceny", 100.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
            (
                repair_ori,
                2024,
                "larceny",
            ...
            (peer_ori, 2024, "larceny", 200.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
            (treated_ori, 2024, "larceny", 10_000.0, 12.0, SUMMARY_SOURCE, "full_monthly"),
        ]
    )
    panel["population"] = 1_000.0
    panel["agency_type_norm"] = "local_police"
    panel["level_policy_resolution"] = "not_applicable"
    panel.loc[
        panel["ori9"].eq(treated_ori) & panel["year"].eq(TARGET_YEAR),
        "level_policy_resolution",
    ] = "admit_valid_complete_year"
    repair = panel["ori9"].eq(repair_ori) & panel["year"].eq(TARGET_YEAR)
    panel.loc[repair, "level1_admission_status"] = "coverage_defective"
    panel.loc[repair, "level2_semantic_status"] = "semantically_unusable"
    panel.loc[repair, "level_repair_mode"] = "decayed_own_history"
    ...
    repaired = estimates[estimates["ori9"].eq(repair_ori)].iloc[0]

    panel.loc[
        panel["ori9"].eq(treated_ori) & panel["year"].eq(TARGET_YEAR),
        "preferred_count",
    ] = 1_000_000.0
    changed_treatment = build_agency_target_estimates_from_panel(
        panel,
        target_year=TARGET_YEAR,
        trend_fill_lookup=TrendFillLookup(state_map={}, national_map={}),
    )
    changed_repair = changed_treatment[changed_treatment["ori9"].eq(repair_ori)].iloc[0]

    assert repaired["agency_estimate_source"] == "decayed_history_hierarchical"
    assert float(changed_repair["estimated_count"]) == pytest.approx(
        float(repaired["estimated_count"])
    )
