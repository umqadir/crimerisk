...
def test_soft_posterior_may_exceed_the_benchmark_residual():
    assert_benchmark_imputation_invariants(_imputation(imputed=9.0, residual=4.0))


def test_a_benchmark_conflict_cell_retains_positive_silent_mass():
    ...
    assert conflicted.imputed_total == 3.0


def test_a_benchmark_conflict_cell_cannot_zero_a_positive_prior():
    conflicted = _imputation(
        imputed=0.0, residual=0.0, conflict_kind="locked_exceeds_benchmark"
    )
    conflicted.units.drop(conflicted.units.index, inplace=True)
    with pytest.raises(ValueError, match="positive silent prior was zeroed"):
        assert_benchmark_imputation_invariants(conflicted)
    ...
    surface = pd.DataFrame(
        {
            ...
            "level_admission_status_burglary": ["valid_complete_year", "coverage_defective"],
            "level_repair_share_burglary": [0.0, 0.0],
            "unresolved_level_flag_burglary": [False, False],
        }
    )
    out = _append_confidence_tiers(surface)
    assert out["confidence_tier_burglary"].tolist() == ["high", "low"]
    assert out["spatial_share_reliability_tier_burglary"].tolist() == ["high", "high"]
    assert out["level_reliability_tier_burglary"].tolist() == ["high", "low"]
...
def test_confidence_provenance_uses_release_year():
    surface = pd.DataFrame(
        {
            "estimate_mode_burglary": ["count_derived"],
            "reliability_tier_burglary": ["high"],
            "source_mode_burglary": [MODELED_SOURCE_MODE],
            "domain_overlap_score_burglary": [1.0],
            "benchmark_imputed_share_burglary": [0.0],
            "level_admission_status_burglary": ["valid_complete_year"],
            "level_repair_share_burglary": [0.0],
            "unresolved_level_flag_burglary": [False],
        }
    )
    out = _append_confidence_tiers(surface, year=2025)
    assert out["level_provenance_text_burglary"].iloc[0] == (
        "2025 total observed from complete official report"
    )
