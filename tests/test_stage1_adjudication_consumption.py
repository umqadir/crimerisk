...
from crimerisk.agency_identity import (
    ...
    build_explicit_succession_ledger,
...
def test_live_registries_declare_only_known_verdicts_and_actions():
    ...
    successions = load_explicit_succession_rulings(paths, target_year=TARGET_YEAR)
    assert not twins.empty and not successions.empty and not zeros.empty and not tokens.empty
    assert not successions["superseded_ori"].duplicated().any()
    assert successions["effective_year"].le(TARGET_YEAR).all()
...
def test_benchmark_supported_predicate_consumes_stage1_usability_directives():
    ...
    panel = pd.DataFrame(
        {
            ...
            "offense": ["larceny", "larceny", "larceny"],
    ...
    assert (reads_missing, "larceny") not in supported
    assert (partial, "larceny") in supported
    assert (ordinary, "larceny") in supported
...
def test_a_new_annual_build_does_not_inherit_prior_year_value_rulings(tmp_path):
    paths = _paths_with_configs(tmp_path)

    assert build_zero_missing_directives(paths, target_year=2025).empty
    assert build_token_reporter_directives(paths, target_year=2025).empty
    assert build_usability_directives(paths, target_year=2025).empty
...
def test_explicit_county_reporter_succession_requires_a_clean_reporting_break():
    rulings = pd.DataFrame(
        [
            {
                "case_id": "mdsp-test",
                "state": "MD",
                "superseded_ori": "MD301SP00",
    ...
    panel = pd.DataFrame(
        [
            ...
            for ori, year, count, months in [
                ("MD301SP00", 2020, 3.0, 12.0),
                ("MD301SP00", 2021, 0.0, 0.0),
                ("MD301SP00", 2024, 0.0, 0.0),
                ("MDMSP2100", 2021, 2.0, 12.0),
                ("MDMSP2100", 2024, 2.0, 12.0),
            ]
        ]
    )
    crosswalk = pd.DataFrame(
        [
            {"ori9": "MD301SP00", "state_fips": "24", "jurisdiction_id": "24:statewide_overlap_layer", "weight": 1.0},
            {"ori9": "MDMSP2100", "state_fips": "24", "jurisdiction_id": "24:municipal:place:2447875", "weight": 0.025},
            {"ori9": "MDMSP2100", "state_fips": "24", "jurisdiction_id": "24:statewide_overlap_layer", "weight": 0.975},
        ]
    )
    master = pd.DataFrame(
        [
            {"ori9": ori, "state_fips": "24", "state_abbr": "MD", "county_fips": "001"}
            for ori in ["MD301SP00", "MDMSP2100"]
        ]
    )
    ledger = build_explicit_succession_ledger(
        rulings,
        agency_panel=panel,
    ...
    assert ledger[["superseded_ori9", "successor_ori9"]].to_dict("records") == [
        {"superseded_ori9": "MD301SP00", "successor_ori9": "MDMSP2100"}
    ]

    broken = panel.copy()
    broken.loc[
        (broken["ori9"].eq("MD301SP00")) & broken["year"].eq(2021),
        "preferred_months_reported",
    ] = 12.0
    with pytest.raises(Stage1AdjudicationError, match="did not stop"):
        build_explicit_succession_ledger(
            rulings,
            agency_panel=broken,
            agency_jurisdiction_crosswalk=crosswalk,
...
def test_the_built_regime_artifact_carries_the_adjudicated_reason_for_exactly_the_ruled_zeros():
    ...
    if int(pd.to_numeric(regimes["year"], errors="coerce").max()) != TARGET_YEAR:
        pytest.skip("built reporting-regime artifact targets a different annual release")
