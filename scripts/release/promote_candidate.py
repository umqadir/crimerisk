...
def _release_parquets(year: int) -> tuple[str, ...]:
    return (
...
def _diagnostic_sidecars(year: int) -> tuple[str, ...]:
    return (
        f"diagnostics/crimerisk_block_group_{year}_cde_exact_sensitivity.parquet",
        f"diagnostics/crimerisk_tract_{year}_cde_exact_sensitivity.parquet",
        f"diagnostics/state_cde_core_comparison_{year}.parquet",
    )


def _audit_sidecars(year: int) -> tuple[str, ...]:
    return (
        ...
        # The unlocated-mass companion table is a RELEASE artifact, not a diagnostic:
        # the release contract's reconciliation identity is
        # `published_block_group_sum + unlocated == control`, and the validator credits
        # the unlocated bucket only when this table is present. Promoting without it
        # left state/output failing four reconciliation assertions by exactly the
        # unlocated amounts while the candidate it was copied from passed clean.
        f"unlocated_mass_{year}.parquet",
    )


def _required_artifacts(year: int) -> tuple[str, ...]:
    return (*_release_parquets(year), *_diagnostic_sidecars(year), *_audit_sidecars(year))


def _stale_superseded_files(year: int) -> tuple[str, ...]:
    return (
        "manifest.json",
...
def _manifest_year(manifest: dict[str, Any]) -> int:
    try:
        year = int(manifest["year"])
    except (KeyError, TypeError, ValueError) as exc:
        raise SystemExit("candidate manifest does not contain a valid integer year") from exc
    if year < 2000 or year > 2100:
        raise SystemExit(f"candidate manifest year is outside the supported range: {year}")
    return year
...
def _frontend_hash_check(
    *,
    ...
    if expected_year is not None:
        try:
            snapshot_year = int(snapshot["data_year"])
        except (KeyError, TypeError, ValueError) as exc:
            raise SystemExit(f"{_repo_relative(snapshot_path)} is missing a valid data_year") from exc
        if snapshot_year != expected_year:
            raise SystemExit(
                "frontend snapshot data year does not match candidate year: "
                f"{snapshot_year} != {expected_year}"
            )
...
def promote_candidate(
    *,
    ...
    candidate_year = _manifest_year(manifest)
    if year is not None and int(year) != candidate_year:
        raise SystemExit(
            f"requested promotion year {year} does not match candidate manifest year {candidate_year}"
        )
    year = candidate_year
    promoted_manifest_name = f"crimerisk_output_build_{year}.json"
    required_artifacts = _required_artifacts(year)
    ...
    promoted_years = {year}
    for old_manifest_path in output_dir.glob("crimerisk_output_build_*.json"):
        try:
            promoted_years.add(int(old_manifest_path.stem.rsplit("_", 1)[-1]))
        except ValueError:
            continue
    stale_names = set(_stale_superseded_files(year))
    for promoted_year in promoted_years:
        stale_names.update(_required_artifacts(promoted_year))
        stale_names.add(f"crimerisk_output_build_{promoted_year}.json")
    ...
    promoted_manifest["promotion"] = {
        ...
        "promoted_output_file_stats": {
            artifact["destination"]: _path_stats(REPO_ROOT / artifact["destination"])
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Expected candidate year. Defaults to the year recorded in the candidate manifest.",
    )
