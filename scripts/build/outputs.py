...
from crimerisk.candidates import normalize_cde_sensitivity_manifest
...
def main() -> int:
    ...
    year = int(args.year)
    output_dir = REPO_ROOT / "state" / "output"
    diagnostics_dir = output_dir / "diagnostics"
    args.bg_ags_core_out = args.bg_ags_core_out or (
        output_dir / f"crimerisk_block_group_{year}_ags_core.parquet"
    )
    args.tract_ags_core_out = args.tract_ags_core_out or (
        output_dir / f"crimerisk_tract_{year}_ags_core.parquet"
    )
    args.bg_fbi_out = args.bg_fbi_out or (
        diagnostics_dir / f"crimerisk_block_group_{year}_cde_exact_sensitivity.parquet"
    )
    args.tract_fbi_out = args.tract_fbi_out or (
        diagnostics_dir / f"crimerisk_tract_{year}_cde_exact_sensitivity.parquet"
    )
    ...
    normalize_cde_sensitivity_manifest(args.build_manifest_out)
    if args.emit_fbi_calibrated:
        comparison_source = paths.state_dir / "controls" / "state_control_comparison.parquet"
        comparison_out = args.bg_fbi_out.parent / f"state_cde_core_comparison_{int(args.year)}.parquet"
