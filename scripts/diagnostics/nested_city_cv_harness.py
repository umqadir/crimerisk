"""Leakage-free nested city-blocked validation for release-year city share transfer.
...
def _initialize_worker(
    ...
    fit_config: CityResidualConfig,
) -> None:
    """Install read-only fit inputs in spawned as well as forked workers."""
    ...
    *,
    ...
    try:
        with ProcessPoolExecutor(
            ...
            initargs=(_BASE_FRAME, _BASE_FEATURE_COLS, _FIT_CONFIG),
        ) as executor:
...
def main() -> int:
    ...
    if args.feature_policy_path is None:
        args.feature_policy_path = paths.state_dir / "modeling" / f"feature_transfer_policy_{year}.parquet"
    if args.prediction_cache is None:
        args.prediction_cache = paths.cache_dir / f"nested_city_cv_prediction_cache_{year}.parquet"
    if args.role_inventory_path is None:
        args.role_inventory_path = paths.state_dir / "modeling" / f"city_role_inventory_{year}.parquet"
    if args.city_shares_path is None:
        args.city_shares_path = (
            paths.state_dir / "modeling" / f"next_phase_validation_city_incident_share_surface_{year}.parquet"
        )
    if args.bg_prior_path is None:
        args.bg_prior_path = paths.state_dir / "modeling" / f"bg_prior_long_{year}.parquet"
    if args.diagnostics_out is None:
        args.diagnostics_out = paths.state_dir / "modeling" / f"nested_city_cv_{year}.parquet"
    if args.summary_json_out is None:
        args.summary_json_out = paths.state_dir / "modeling" / f"nested_city_cv_{year}.json"
    ...
    summary = {
        ...
        "upstream_baseline_scope": {
            f"bg_prior_long_{year}": str(args.bg_prior_path),
        ...
        "supersedes_as_heldout_evidence": [
            f"state/modeling/city_share_benchmark_{year}.*",
            f"state/modeling/city_residual_benchmark_{year}.*",
            f"state/modeling/next_phase_city_residual_benchmark_{year}.*",
            f"state/modeling/share_temper_tau_calibration_{year}.json",
        ],
    }
