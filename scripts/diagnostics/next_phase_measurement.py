...
def build_jurisdiction_cv_predictions(
    *,
    ...
    for offense in OFFENSES_7:
        ...
        # This is a status field for the validation table, not a dominant source
        # label.  The training code deliberately removes ``preferred_source``:
        # one label cannot represent a jurisdiction control assembled from
        # multiple agencies or source lanes and must never drive a decision.
        train_meta["production_estimate_source"] = np.select(
            [
                _bool_series(train_meta["needs_current_year_fill"]),
                _bool_series(train_meta["estimated_from_panel"]),
            ],
            ["current_year_fill", "panel_estimate"],
            default="current_year_control",
        )
...
def _bg_population_frame(surface_path: Path, *, year: int) -> pd.DataFrame:
...
def _build_output_count_prior(output: pd.DataFrame, *, offenses: Iterable[str]) -> pd.DataFrame:
    """Put published expected counts into the allocation-benchmark prior schema."""
    identity = ["bg_id", "tract_id", "state_fips"]
    frames = [
        output[[*identity, f"expected_count_{offense}"]]
        .rename(columns={f"expected_count_{offense}": "bg_weight"})
        .assign(offense=str(offense))
        for offense in offenses
        if f"expected_count_{offense}" in output.columns
    ]
    if not frames:
        return pd.DataFrame(columns=[*identity, "bg_weight", "offense"])
...
def build_allocation_measurement_tables(
    *,
    ...
    bg_population = _bg_population_frame(surface_path, year=year)
    output = pd.read_parquet(surface_path)
    ...
    output_long = _build_output_count_prior(output, offenses=offenses)
...
def main() -> int:
    ...
    parser.add_argument(
        "--allocation-surface-path",
        type=Path,
        default=None,
        help=(
            "Target-year block-group surface used for population and the existing-output "
            "allocation comparator. Defaults to the promoted state/output surface."
        ),
    )
    parser.add_argument(
    ...
    allocation_surface_path = args.allocation_surface_path or (
        REPO_ROOT
        / "state"
        / "output"
    ...
    # This fit dominates runtime. Persist it before the independent allocation/report
    # section so a downstream input-contract failure cannot discard completed CV work.
    cv_predictions_out.parent.mkdir(parents=True, exist_ok=True)
    ...
    bg_diag, tract_diag, allocation_summary = build_allocation_measurement_tables(
        ...
        bg_prior=bg_prior,
        surface_path=allocation_surface_path,
    )
