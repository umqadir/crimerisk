...
def _policy_group_for_feature(column: str) -> str:
    ...
    # ACS-derived feature names carry the edition year.  Policy classes describe
    # the variable, not one vintage, so strip only a terminal four-digit year
    # before matching the small set of annual aliases below.
    annual_base = re.sub(r"_20\d{2}$", "", col)
...
def _build_feature_inventory(
    *,
    ...
    if "feature_source" not in features.columns:
        features["feature_source"] = pd.NA
